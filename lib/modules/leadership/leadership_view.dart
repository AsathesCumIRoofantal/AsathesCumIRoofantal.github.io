...
class LeadershipView extends GetView<LeadershipController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('LEADERSHIP', style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold, fontSize: 14)),
        centerTitle: true,
        actions: [Padding(
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildSectionLabel('LEADERSHIP POSTURE', Icons.track_changes_outlined, _blue, onSurface),
          const SizedBox(height: 12),
          _buildPostureGrid(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('LEADERSHIP TOOLS', Icons.build_rounded, _sky, onSurface),
          const SizedBox(height: 12),
          _buildLeaderCard(context, isDark, Icons.track_changes_outlined, _blue, 'Vision Setting',
          ...
          _buildLeaderCard(context, isDark, Icons.account_tree_outlined, _cyan, 'Delegation Map',
          ...
          const SizedBox(height: 10),
          _buildLeaderCard(context, isDark, Icons.fact_check_outlined, _green, 'Decision Log',
          ...
          const SizedBox(height: 10),
          _buildLeaderCard(context, isDark, Icons.checklist_outlined, _amber, 'Follow-Through Tracker',
          ...
          const SizedBox(height: 10),
          _buildLeaderCard(context, isDark, Icons.rate_review_outlined, _violet, 'Feedback Cadence',
          ...
          const SizedBox(height: 10),
          _buildLeaderCard(context, isDark, Icons.menu_book_outlined, _sky, 'Leadership Principles',
          ...
          const SizedBox(height: 10),
          _buildLeaderCard(context, isDark, Icons.bolt_outlined, _amber, 'Energy & Presence',
  ...
  Widget _buildHeroHeader(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: _blue.withOpacity(0.38)),
        boxShadow: [BoxShadow(color: _blue.withOpacity(0.14), blurRadius: 20)],
      ),
      child: Column(children: [
        Row(children: [
          Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(
            color: _blue.withOpacity(0.15), borderRadius: BorderRadius.circular(14)),
            child: const Icon(Icons.military_tech_outlined, color: _sky, size: 28)),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('LEADERSHIP COMMAND CENTRE', style: TextStyle(fontSize: 9, color: _sky, fontWeight: FontWeight.bold, letterSpacing: 2)),
            const SizedBox(height: 4),
            const Text('Leadership in AIR is about setting clear direction and following through with accountability — from vision-casting to delegation and execution.',
  ...
  Widget _buildPostureGrid(BuildContext context, bool isDark, Color onSurface) {
    final postures = [
      ...
      (Icons.account_tree_rounded, 'Delegator', 'Matches tasks to talent', _cyan),
    ...
    return GridView.count(
      ...
      children: postures.map((p) => Container(
        padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
        decoration: BoxDecoration(
          color: p.$4.withOpacity(0.07), borderRadius: BorderRadius.circular(14),
          border: Border.all(color: p.$4.withOpacity(0.22)),
        ),
        child: Row(children: [
          Icon(p.$1, color: p.$4, size: 20),
          const SizedBox(width: 8),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
            Text(p.$2, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: onSurface)),
            Text(p.$3, style: TextStyle(fontSize: 9, color: onSurface.withOpacity(0.55))),
          ])),
        ]),
      )).toList(),
    );
  ...
  Widget _buildLeaderCard(BuildContext context, bool isDark, IconData icon, Color color, String title, String body) {
    return Container(
  ...
  Widget _buildLeadershipQuote(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_blue.withOpacity(0.10), _violet.withOpacity(0.07)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
