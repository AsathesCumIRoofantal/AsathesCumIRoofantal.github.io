...
## 9. PATCH 2026-07-16 — Multi-screen-share (10 sharers) + dark-theme button-visibility fix

### What you asked for
Let up to 10 people share their screen at once, with any number of people
able to view, and fix buttons/text that were invisible because their
foreground and background colors matched.

### What changed — screen sharing

- **Real bug fixed:** `ZoomMeetingController` used to force-clear *every
  other* participant's `isScreenSharing` flag the instant anyone started
  sharing — so a second presenter starting their share silently killed
  the first presenter's shared view for everyone, even though nothing was
  wrong at the connection level. Multiple people can now share
  simultaneously; each person's share is fully independent at the engine
  layer already (screen-share is a per-connection track replace, scoped
  to that user's own peer connections — see `webrtc_service.dart`).
- Added a **10-concurrent-sharer cap**
  (`ZoomMeetingController.maxConcurrentScreenShares`), enforced in two
  places: the toolbar button (so you don't even see the OS share picker
  once the cap is hit) and, more importantly, **in the database** — see
  `supabase_schema.sql`'s 2026-07-16 patch, section A. The client-side
  check alone only protects an honest client; the DB trigger makes the
  limit real regardless of which client is asking.
- Added a dedicated screen-share stage layout
  (`_ScreenShareStage` in `zoom_meeting_view.dart`): one sharer gets the
  classic big single view; 2+ sharers get an even grid, with everyone
  else's camera collapsed into a thin strip so the shared screens keep
  the room. Previously there was no screen-share-aware layout at all —
  a shared screen only showed up if that person also happened to be
  picked as "active speaker," which was accidental at best.

### The honest part: "any number of viewers"

This is still a **zero-cost mesh** build (see §5 above) — there is no
media server sitting between people. That means every presenter encodes
and uploads their screen **once per other person in the meeting**, not
once total. Concretely: 10 people presenting to a room of 100 means each
of those 10 presenters is uploading 99 separate video streams at the same
time — that's not a software limit I can raise, it's home-upload-bandwidth
physics. Capping *presenters* at 10 was a real, useful thing to build;
promising unlimited *viewers* on top of mesh would not be honest.

If you actually need "10 presenters, hundreds of viewers" (a real webinar
shape), that requires the SFU (LiveKit) path mentioned in §5 — happy to
scaffold that next if it's actually where you're headed. Until then, a
realistic total-room ceiling on this mesh build is still in the same
rough territory as the existing 6-8-simultaneous-video guidance, a bit
higher if most non-presenters keep their cameras off (camera-off
participants still cost each presenter upload bandwidth, just less than
a full video tile would).

### What changed — the invisible-button bug

The actual bug: this module is dropped into *your* `GetMaterialApp`, and
several widgets (unstyled `DropdownButton` items, default `TextButton`/
`ElevatedButton` labels, the Settings `AppBar`) never set an explicit
color — they read from whatever `ThemeData` your host app around it
uses. On a light host theme, several of those ambient defaults land at
or near black, which on this module's near-black backgrounds (`ZoomTheme.bg`,
`surface`) made text and controls technically present but effectively
invisible — most noticeably the Settings screen's noise-suppression/theme
dropdowns and its `AppBar`.

Fixed at the root instead of widget-by-widget: every route this module
owns is now wrapped in a forced `ZoomTheme.themeData` (see
`zoom_routes.dart` / `widgets/zoom_theme.dart`), so nothing in here can
silently inherit an invisible color combination from your app's theme
again, however your app is themed. Settings screen's `Dropdown` items and
`AppBar` also got explicit colors on top of that, for defense in depth.

### Things to do on your side
1. Re-run `supabase_schema.sql` in the Supabase SQL editor (just the new
   2026-07-16 section at the bottom — it's additive/idempotent, safe to
   run on top of what you already ran).
2. Set the two database settings the R2-cleanup cron job needs
   (`app.supabase_url`, `app.service_role_key`) — see the comment above
   that job in the SQL file. Without them that one cron job will error
   every run; nothing else depends on it.
3. If you actually need hundreds of viewers rather than dozens, tell me
   and I'll scaffold the LiveKit/SFU path from §5 — it's a separate,
   sizeable piece of work (a server to deploy, plus a new
   `RtcEngineInterface` implementation), not something to bolt on
   silently.
4. Test with 3+ real devices/browsers sharing screens at the same time —
   I couldn't run Flutter in the sandbox this was built in (no SDK/
   network), so this is code-reviewed and logically verified, not
   device-tested. Send me any errors you hit.
