...
class HeartWinningView extends GetView<HeartWinningController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('HEART WINNING', style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold, fontSize: 14)),
        centerTitle: true,
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildSectionLabel('LEADERSHIP DIMENSIONS', Icons.favorite, _rose, onSurface),
          const SizedBox(height: 12),
          _buildDimGrid(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('HEART-CENTRED TOOLS', Icons.build_rounded, _orange, onSurface),
          const SizedBox(height: 12),
          _buildCard(context, isDark, Icons.favorite, _rose, 'Empathetic Leadership',
          ...
          _buildCard(context, isDark, Icons.groups, _orange, 'Community Engagement',
          ...
          const SizedBox(height: 10),
          _buildCard(context, isDark, Icons.policy, _blue, 'Policy with Purpose',
          ...
          const SizedBox(height: 10),
          _buildCard(context, isDark, Icons.hearing, _teal, 'Listening Sessions',
          ...
          const SizedBox(height: 10),
          _buildCard(context, isDark, Icons.star_outline, _amber, 'Recognition & Gratitude',
          ...
          const SizedBox(height: 10),
          _buildCard(context, isDark, Icons.handshake, _violet, 'Conflict Resolution',
          ...
          const SizedBox(height: 10),
          _buildCard(context, isDark, Icons.auto_stories, _teal, 'Impact Stories',
  ...
  Widget _buildHeroHeader(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [const Color(0xFF38000A), const Color(0xFF1C0005)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(22),
        border: Border.all(color: _rose.withOpacity(0.38)),
        boxShadow: [BoxShadow(color: _crimson.withOpacity(0.14), blurRadius: 16)],
      ),
      child: Column(children: [
        Row(children: [
          Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(
            color: _rose.withOpacity(0.15), borderRadius: BorderRadius.circular(14)),
            child: const Icon(Icons.volunteer_activism, color: _rose, size: 28)),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('EMPATHETIC PUBLIC LEADERSHIP', style: TextStyle(fontSize: 9, color: _rose, fontWeight: FontWeight.bold, letterSpacing: 2)),
            const SizedBox(height: 4),
            const Text('Winning hearts means delivering results while treating every person with dignity and respect. Sound policy and genuine empathy are not opposites — the best leaders master both.',
    ...
    return GridView.count(
      ...
      children: dims.map((d) => Container(
        ...
        child: Row(children: [
          ...
          Text(d.$2, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: onSurface)),
        ]),
      )).toList(),
    );
  }
  ...
  Widget _buildHeartQuote(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_rose.withOpacity(0.10), _teal.withOpacity(0.07)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
