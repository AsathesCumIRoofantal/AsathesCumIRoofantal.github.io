...
-- ════════════════════════════════════════════════════════════
--  PATCH 2026-08-27 — Community RLS id-mismatch fix + R2 upload_file
--  Run this block ONCE in the Supabase SQL Editor. Idempotent.
--
--  Context: CurrentUser.id (used everywhere in the Flutter module as
--  senderId / authorId / memberIds / userId) resolves to
--  `user_table.user_id`, NOT `auth.uid()` (see services/current_user.dart —
--  the two are linked only through user_table.auth_user_id). Every table
--  patched in 2026-07-06/07-16 (meeting_participants, meeting_pre_settings,
--  social_posts, social_reactions, social_comments) already resolves the
--  caller's id through the
--    (SELECT user_id FROM user_table WHERE auth_user_id = auth.uid()::uuid LIMIT 1)
--  subquery. chat_rooms and chat_messages were never updated to that
--  pattern — their policies (section 14, original schema) still compare
--  auth.uid() directly against member_ids / admin_ids / sender_id, which
--  are user_table.user_id values. Since auth.uid() != user_table.user_id
--  for any real user, EVERY community/DM/group read and write is silently
--  rejected by RLS — CommunityService.listRooms() returns zero rows,
--  createRoom()/sendMessage() throw a Postgres RLS error. This was not
--  caught earlier because nothing before this drop (community/ + social/
--  folders) actually exercised chat_rooms/chat_messages from the client.
-- ════════════════════════════════════════════════════════════

-- ────────────────────────────────────────────────────────────
--  A. chat_rooms: fix SELECT/UPDATE to resolve id via user_table,
--     and add the INSERT policy that never existed (createRoom()
--     was always going to fail RLS with "no policy" even after
--     the id-mismatch above is fixed).
-- ────────────────────────────────────────────────────────────
DROP POLICY IF EXISTS chat_rooms_member_select ON chat_rooms;
CREATE POLICY chat_rooms_member_select ON chat_rooms
  FOR SELECT USING (
...
CREATE POLICY chat_rooms_insert ON chat_rooms
  FOR INSERT WITH CHECK (
    created_by = (SELECT user_id FROM user_table WHERE auth_user_id = auth.uid()::uuid LIMIT 1)
    AND (SELECT user_id FROM user_table WHERE auth_user_id = auth.uid()::uuid LIMIT 1)
      = ANY(member_ids)
  );

-- ────────────────────────────────────────────────────────────
--  B. chat_messages: same id-mismatch fix for select/insert/update.
-- ────────────────────────────────────────────────────────────
DROP POLICY IF EXISTS chat_msg_select ON chat_messages;
CREATE POLICY chat_msg_select ON chat_messages
  FOR SELECT USING (
...
CREATE POLICY chat_msg_insert ON chat_messages
  FOR INSERT WITH CHECK (
    sender_id = (SELECT user_id FROM user_table WHERE auth_user_id = auth.uid()::uuid LIMIT 1)
  );
...
-- ────────────────────────────────────────────────────────────
--  C. social_reactions: add the missing UPDATE policy.
--     SocialService.reactToPost() does an upsert(onConflict:
--     'target_id,user_id') — changing an existing reaction's emoji
--     compiles to INSERT ... ON CONFLICT DO UPDATE, which needs an
--     UPDATE policy in addition to INSERT. Only SELECT/INSERT/DELETE
--     existed, so a user could react once but never change the emoji
--     (silently fails / 0 rows affected).
-- ────────────────────────────────────────────────────────────
DROP POLICY IF EXISTS sr_update_own ON social_reactions;
CREATE POLICY sr_update_own ON social_reactions
  FOR UPDATE USING (
    user_id = (SELECT user_id FROM user_table WHERE auth_user_id = auth.uid()::uuid LIMIT 1)
  );

-- ────────────────────────────────────────────────────────────
--  D. temp_files RLS has the same auth.uid()-vs-user_table.user_id
--     mismatch as (A)/(B) above. Left AS-IS deliberately: no client
--     code in this drop inserts into temp_files directly (only the
--     recording_manager Edge Function would, and Edge Functions use
--     the service_role key, which bypasses RLS entirely). Fixing it
--     costs nothing and is the safe/consistent thing to do, so it's
--     included — but note it was not an active bug.
-- ────────────────────────────────────────────────────────────
DROP POLICY IF EXISTS temp_files_select ON temp_files;
CREATE POLICY temp_files_select ON temp_files
  FOR SELECT USING (
...
-- ────────────────────────────────────────────────────────────
--  E. Not a SQL fix (nothing to run here) — flagging for the
--     matching code change: R2UploadService.uploadFile() (used by
--     community_room_view.dart attach, post_create_view.dart media
--     upload, and in_meeting/chat_panel.dart file attach) calls the
--     recording_manager Edge Function with { action: "upload_file" },
--     but supabase_functions/recording_manager/index.ts only
--     implements start / getPartUrl / complete / abort / pause /
--     resume. Every attach/upload in Community, Social, and in-meeting
--     chat will 400 with "Unknown action: upload_file" until that
--     case is added to the function (see the corrected
--     recording_manager/index.ts delivered alongside this file, and
--     Step 5 of todolist.md).
-- ────────────────────────────────────────────────────────────

-- END OF PATCH 2026-08-27
