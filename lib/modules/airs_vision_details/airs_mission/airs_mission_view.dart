...
class AirsMissionView extends GetView<AirsMissionController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("AIR'S MISSION", style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold, fontSize: 14)),
        centerTitle: true,
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildSectionLabel('CORE MISSION', Icons.stars_rounded, _flag, onSurface),
          const SizedBox(height: 12),
          _buildMissionStatement(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('STRATEGIC PRIORITIES', Icons.format_list_numbered_rounded, _blue, onSurface),
          const SizedBox(height: 12),
          _buildPriorityList(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('MISSION DETAILS', Icons.auto_stories_rounded, _cyan, onSurface),
          const SizedBox(height: 12),
          _buildMissionCard(context, isDark, Icons.gavel_rounded, _flag, 'Non-Negotiable Commitments',
          ...
          _buildMissionCard(context, isDark, Icons.checklist_rounded, _blue, 'Mission Alignment Check',
          ...
          const SizedBox(height: 10),
          _buildMissionCard(context, isDark, Icons.handshake_rounded, _teal, 'Stakeholder Commitments',
          ...
          const SizedBox(height: 10),
          _buildMissionCard(context, isDark, Icons.history_edu_rounded, _violet, 'Mission History & Evolution',
          ...
          const SizedBox(height: 10),
          _buildMissionCard(context, isDark, Icons.format_list_numbered_rounded, _green, 'Strategic Priorities',
          ...
          const SizedBox(height: 10),
          _buildMissionCard(context, isDark, Icons.stars_rounded, _amber, 'Core Mission Statement',
  ...
  Widget _buildHeroHeader(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [const Color(0xFF01091E), const Color(0xFF010510)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(22),
        border: Border.all(color: _flag.withOpacity(0.38)),
        boxShadow: [BoxShadow(color: _blue.withOpacity(0.14), blurRadius: 18)],
      ),
      child: Row(children: [
        Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(
          color: _flag.withOpacity(0.15), borderRadius: BorderRadius.circular(14)),
          child: const Icon(Icons.flag_rounded, color: _flag, size: 28)),
        const SizedBox(width: 14),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('WHY AIR EXISTS', style: TextStyle(fontSize: 10, color: _flag, fontWeight: FontWeight.bold, letterSpacing: 2)),
          const SizedBox(height: 4),
          const Text('Understand the non-negotiable commitments and strategic priorities that define what AIR stands for. The mission is the decision filter applied every time AIR faces a trade-off.',
  ...
  Widget _buildMissionStatement(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      padding: const EdgeInsets.all(18),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_blue.withOpacity(0.12), _cyan.withOpacity(0.08)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: _flag.withOpacity(0.28)),
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('CORE MISSION', style: TextStyle(fontSize: 9, color: _flag, fontWeight: FontWeight.bold, letterSpacing: 2)),
        const SizedBox(height: 10),
        Text(
          'AIR exists to revolutionise industrial coordination through autonomous digitisation and community-driven verification.',
          style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, height: 1.4, color: onSurface),
        ),
        const SizedBox(height: 8),
        Text('Every feature, policy, and partnership is evaluated against this statement before it is approved.',
            style: TextStyle(fontSize: 12, color: onSurface.withOpacity(0.65), height: 1.4)),
      ]),
    );
  }

  Widget _buildPriorityList(BuildContext context, bool isDark, Color onSurface) {
    final priorities = [
    ...
    return Column(children: priorities.asMap().entries.map((e) => Container(
      margin: const EdgeInsets.only(bottom: 8),
      padding: const EdgeInsets.all(13),
      decoration: BoxDecoration(
        color: e.value.$2.withOpacity(0.06), borderRadius: BorderRadius.circular(12),
        border: Border.all(color: e.value.$2.withOpacity(0.18)),
      ),
      child: Row(children: [
        Container(width: 22, height: 22,
          decoration: BoxDecoration(shape: BoxShape.circle, color: e.value.$2.withOpacity(0.15)),
          child: Center(child: Text('${e.key + 1}', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: e.value.$2)))),
        const SizedBox(width: 10),
        Expanded(child: Text(e.value.$1, style: TextStyle(fontSize: 11, color: onSurface, height: 1.3))),
      ]),
    )).toList());
  }

  Widget _buildMissionCard(BuildContext context, bool isDark, IconData icon, Color color, String title, String body) {
    return Container(
  ...
  Widget _buildMissionQuote(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_flag.withOpacity(0.10), _violet.withOpacity(0.07)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: _flag.withOpacity(0.20)),
      ),
      child: Row(children: [
