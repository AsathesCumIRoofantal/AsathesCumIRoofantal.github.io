...
class AccountableView extends GetView<AccountableController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('ACCOUNTABLE', style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold, fontSize: 14)),
        centerTitle: true,
        actions: [Padding(
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildSectionLabel('COMMITMENT LEDGER', Icons.menu_book_rounded, _blue, onSurface),
          const SizedBox(height: 12),
          _buildLedgerPanel(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('ACCOUNTABILITY SCORE', Icons.score_rounded, _green, onSurface),
          const SizedBox(height: 12),
          _buildScoreCard(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('ACCOUNTABILITY TOOLS', Icons.build_rounded, _violet, onSurface),
          const SizedBox(height: 12),
          _buildAcctCard(context, isDark, Icons.assignment_ind_rounded, _blue, 'Outcome Ownership Declaration',
          ...
          _buildAcctCard(context, isDark, Icons.update_rounded, _sky, 'Transparent Progress Updates',
          ...
          const SizedBox(height: 10),
          _buildAcctCard(context, isDark, Icons.report_problem_rounded, _amber, 'Miss & Learn Report',
          ...
          const SizedBox(height: 10),
          _buildAcctCard(context, isDark, Icons.menu_book_rounded, _green, 'Commitment Ledger',
          ...
          const SizedBox(height: 10),
          _buildAcctCard(context, isDark, Icons.swap_horiz_rounded, _violet, 'Delegation Handoff Protocol',
          ...
          const SizedBox(height: 10),
          _buildAcctCard(context, isDark, Icons.check_circle_rounded, _green, 'Closure Ritual',
  ...
  Widget _buildHeroHeader(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: _blue.withOpacity(0.12), blurRadius: 16)],
      ),
      child: Column(children: [
        Row(children: [
          Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(
            color: _blue.withOpacity(0.15), borderRadius: BorderRadius.circular(14)),
            child: const Icon(Icons.fact_check_rounded, color: _sky, size: 28)),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('ACCOUNTABILITY CENTRE', style: TextStyle(fontSize: 10, color: _sky, fontWeight: FontWeight.bold, letterSpacing: 2)),
            const SizedBox(height: 4),
            const Text('Own every outcome you set in motion — through transparent ledgers, kept promises, and honest follow-through even when no one is watching.',
    ...
    return Container(
      ...
      child: Column(children: entries.asMap().entries.map((e) {
        ...
        return Container(
          ...
          child: Row(children: [
            Icon(item.$3 == _green ? Icons.check_circle_rounded : item.$3 == _red ? Icons.cancel_rounded : Icons.radio_button_unchecked_rounded,
                color: item.$3, size: 18),
            const SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(item.$1, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: onSurface)),
              Text(item.$2, style: TextStyle(fontSize: 10, color: onSurface.withOpacity(0.55))),
            ])),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
              decoration: BoxDecoration(color: item.$3.withOpacity(0.12), borderRadius: BorderRadius.circular(6)),
              child: Text(item.$4, style: TextStyle(fontSize: 8, fontWeight: FontWeight.bold, color: item.$3, letterSpacing: 1)),
            ),
          ]),
        );
      }).toList()),
  ...
  Widget _buildScoreCard(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: _green.withOpacity(0.07), borderRadius: BorderRadius.circular(16),
        border: Border.all(color: _green.withOpacity(0.22)),
      ),
      child: Row(children: [
        Stack(alignment: Alignment.center, children: [
          SizedBox(width: 60, height: 60, child: CircularProgressIndicator(
            value: 0.97, strokeWidth: 5,
            backgroundColor: Colors.white.withOpacity(0.08),
            color: _green,
          )),
          const Text('97%', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: _green)),
        ]),
        const SizedBox(width: 16),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Accountability Score', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: onSurface)),
          const SizedBox(height: 4),
          Text('Your rolling 90-day commitment-kept ratio. Private by default — share selectively to build credibility.',
              style: TextStyle(fontSize: 11, height: 1.3, color: onSurface.withOpacity(0.65))),
  ...
  Widget _buildAcctCard(BuildContext context, bool isDark, IconData icon, Color color, String title, String body) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
  ...
  Widget _buildAccountabilityQuote(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_blue.withOpacity(0.10), _green.withOpacity(0.07)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(16),
