...
class LoyaltyView extends GetView<LoyaltyController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('LOYALTY', style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold, fontSize: 14)),
        centerTitle: true,
        actions: [Padding(
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildSectionLabel('COMMITMENT TYPES', Icons.category_rounded, _indigo, onSurface),
          const SizedBox(height: 12),
          _buildCommitmentTypes(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('LOYALTY PRINCIPLES', Icons.anchor_rounded, _violet, onSurface),
          const SizedBox(height: 12),
          _buildLoyaltyCard(context, isDark, Icons.anchor_outlined, _indigo, 'Long-Term Commitments',
          ...
          _buildLoyaltyCard(context, isDark, Icons.science_outlined, _sky, 'Experimental Allegiances',
          ...
          const SizedBox(height: 10),
          _buildLoyaltyCard(context, isDark, Icons.thumb_up_alt_outlined, _green, 'Loyalty Signals',
          ...
          const SizedBox(height: 10),
          _buildLoyaltyCard(context, isDark, Icons.warning_outlined, _amber, 'Loyalty vs. Enabling',
          ...
          const SizedBox(height: 10),
          _buildLoyaltyCard(context, isDark, Icons.compare_arrows_outlined, _blue, 'Reciprocal Loyalty Check',
          ...
          const SizedBox(height: 10),
          _buildLoyaltyCard(context, isDark, Icons.exit_to_app_outlined, _rose, 'Exiting with Integrity',
  ...
  Widget _buildHeroHeader(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: _violet.withOpacity(0.38)),
        boxShadow: [BoxShadow(color: _violet.withOpacity(0.12), blurRadius: 18)],
      ),
      child: Column(children: [
        Row(children: [
          Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(
            color: _violet.withOpacity(0.15), borderRadius: BorderRadius.circular(14)),
            child: const Icon(Icons.verified_outlined, color: _violet, size: 28)),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('DELIBERATE LOYALTY', style: TextStyle(fontSize: 10, color: _violet, fontWeight: FontWeight.bold, letterSpacing: 2)),
            const SizedBox(height: 4),
            const Text('Loyalty in AIR is about knowing where you commit long-term versus where you experiment — not blind allegiance, but a deliberate signal of deep investment.',
  ...
  Widget _buildCommitmentTypes(BuildContext context, bool isDark, Color onSurface) {
    return Row(children: [
      Expanded(child: Container(
        padding: const EdgeInsets.all(14),
        decoration: BoxDecoration(
          color: _indigo.withOpacity(0.08), borderRadius: BorderRadius.circular(14),
          border: Border.all(color: _indigo.withOpacity(0.22)),
        ),
        child: Column(children: [
          const Icon(Icons.anchor_rounded, color: _indigo, size: 24),
          const SizedBox(height: 8),
          Text('Long-term', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: onSurface)),
          const SizedBox(height: 4),
          Text('Unconditional investment — you stay through difficulty', textAlign: TextAlign.center,
              style: TextStyle(fontSize: 9, color: onSurface.withOpacity(0.60), height: 1.3)),
        ]),
      )),
      ...
      Expanded(child: Container(
        padding: const EdgeInsets.all(14),
        decoration: BoxDecoration(
          color: _sky.withOpacity(0.08), borderRadius: BorderRadius.circular(14),
          border: Border.all(color: _sky.withOpacity(0.22)),
        ),
        child: Column(children: [
          const Icon(Icons.science_rounded, color: _sky, size: 24),
          const SizedBox(height: 8),
          Text('Trial-period', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: onSurface)),
          const SizedBox(height: 4),
          Text('Time-boxed investment — honest evaluation at the end', textAlign: TextAlign.center,
              style: TextStyle(fontSize: 9, color: onSurface.withOpacity(0.60), height: 1.3)),
        ]),
      )),
    ]);
  ...
  Widget _buildLoyaltyCard(BuildContext context, bool isDark, IconData icon, Color color, String title, String body) {
    return Container(
  ...
  Widget _buildLoyaltyMap(BuildContext context, bool isDark, Color onSurface) {
    final relationships = [
      ('Family', 'Unconditional — core anchor', Icons.family_restroom_rounded, _rose),
      ('Core Team', 'Long-term — invested through difficulty', Icons.groups_rounded, _indigo),
      ('AIR Community', 'Long-term — shared mission', Icons.public_rounded, _violet),
      ('New Partnership', 'Trial — 6 months remaining', Icons.handshake_rounded, _sky),
    ];
    return Column(children: relationships.map((r) => Container(
      margin: const EdgeInsets.only(bottom: 8),
      padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 11),
      decoration: BoxDecoration(
        color: r.$4.withOpacity(0.06), borderRadius: BorderRadius.circular(12),
        border: Border.all(color: r.$4.withOpacity(0.18)),
      ),
      child: Row(children: [
        Icon(r.$3, color: r.$4, size: 20),
        const SizedBox(width: 10),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(r.$1, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: onSurface)),
          Text(r.$2, style: TextStyle(fontSize: 10, color: onSurface.withOpacity(0.58))),
        ])),
  ...
  Widget _buildLoyaltyQuote(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_violet.withOpacity(0.10), _indigo.withOpacity(0.07)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: _violet.withOpacity(0.20)),
      ),
      child: Row(children: [
