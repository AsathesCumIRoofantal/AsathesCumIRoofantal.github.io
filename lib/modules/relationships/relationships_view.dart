...
class RelationshipsView extends GetView<RelationshipsController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('RELATIONSHIPS', style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold, fontSize: 14)),
        centerTitle: true,
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildSectionLabel('NETWORK OVERVIEW', Icons.hub_rounded, _hub, onSurface),
          const SizedBox(height: 12),
          _buildNetworkStats(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('RELATIONSHIP TYPES', Icons.category_rounded, _blue, onSurface),
          const SizedBox(height: 12),
          _buildRelTypeGrid(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('RELATIONSHIP TOOLS', Icons.build_rounded, _teal, onSurface),
          const SizedBox(height: 12),
          _buildRelCard(context, isDark, Icons.account_tree_outlined, _hub, 'Relationship Map',
          ...
          _buildRelCard(context, isDark, Icons.school_outlined, _violet, 'Mentors & Sponsors',
          ...
          const SizedBox(height: 10),
          _buildRelCard(context, isDark, Icons.group_outlined, _blue, 'Peers & Collaborators',
          ...
          const SizedBox(height: 10),
          _buildRelCard(context, isDark, Icons.family_restroom, _green, 'Dependents & Responsibilities',
          ...
          const SizedBox(height: 10),
          _buildRelCard(context, isDark, Icons.diversity_3, _teal, 'Allies & Advocates',
          ...
          const SizedBox(height: 10),
          _buildRelCard(context, isDark, Icons.favorite_border, _rose, 'Relationship Health Score',
          ...
          const SizedBox(height: 10),
          _buildRelCard(context, isDark, Icons.chat_bubble_outline, _amber, 'Interaction Log',
  ...
  Widget _buildHeroHeader(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [const Color(0xFF001A33), const Color(0xFF000D1A)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(22),
        border: Border.all(color: _hub.withOpacity(0.35)),
        boxShadow: [BoxShadow(color: _blue.withOpacity(0.12), blurRadius: 16)],
      ),
      child: Column(children: [
        Row(children: [
          Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(
            color: _hub.withOpacity(0.15), borderRadius: BorderRadius.circular(14)),
            child: const Icon(Icons.hub_outlined, color: _hub, size: 28)),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('RELATIONSHIP INTELLIGENCE', style: TextStyle(fontSize: 9, color: _hub, fontWeight: FontWeight.bold, letterSpacing: 2)),
            const SizedBox(height: 4),
            const Text('Map every important tie — mentors, peers, dependents, allies — so you understand your full network. AIR helps you nurture high-value connections and spot gaps before they become liabilities.',
  ...
  Widget _buildNetworkStats(BuildContext context, bool isDark, Color onSurface) {
    final metrics = [
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: _hub.withOpacity(0.14)),
      ),
      child: Column(children: metrics.map((m) => Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Text('${(m.$2 * 100).round()}%', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: m.$3)),
          ]),
          const SizedBox(height: 4),
          Stack(children: [
  ...
  Widget _buildRelTypeGrid(BuildContext context, bool isDark, Color onSurface) {
    ...
    return GridView.count(
      ...
      children: types.map((t) => Container(
        ...
        decoration: BoxDecoration(
          color: t.$4.withOpacity(0.07), borderRadius: BorderRadius.circular(12),
          border: Border.all(color: t.$4.withOpacity(0.20)),
        ),
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(t.$1, color: t.$4, size: 18),
          const SizedBox(height: 5),
          Text(t.$3, style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: t.$4)),
          Text(t.$2, style: TextStyle(fontSize: 9, color: onSurface.withOpacity(0.60))),
        ]),
      )).toList(),
    );
  }

  Widget _buildRelCard(BuildContext context, bool isDark, IconData icon, Color color, String title, String body) {
    return Container(
  ...
  Widget _buildRelationshipQuote(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_hub.withOpacity(0.10), _violet.withOpacity(0.07)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: _hub.withOpacity(0.20)),
      ),
      child: Row(children: [
