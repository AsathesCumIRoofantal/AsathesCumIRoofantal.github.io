...
class CommunicationView extends GetView<CommunicationController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('COMMUNICATION', style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold, fontSize: 14)),
        centerTitle: true,
        actions: [Padding(
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildSectionLabel('ACTIVE CHANNELS', Icons.hub_rounded, _sky, onSurface),
          const SizedBox(height: 12),
          _buildChannelGrid(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('COMMUNICATION HEALTH', Icons.health_and_safety_rounded, _blue, onSurface),
          const SizedBox(height: 12),
          _buildHealthRow(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('CAPABILITIES & SETTINGS', Icons.settings_rounded, _indigo, onSurface),
          const SizedBox(height: 12),
          _buildCapabilityCard(context, isDark, Icons.tune_rounded, _blue, 'Tone & Cadence Settings',
          ...
          _buildCapabilityCard(context, isDark, Icons.description_rounded, _indigo, 'Message Templates',
          ...
          const SizedBox(height: 10),
          _buildCapabilityCard(context, isDark, Icons.campaign_rounded, _cyan, 'Broadcast & Announcements',
          ...
          const SizedBox(height: 10),
          _buildCapabilityCard(context, isDark, Icons.history_rounded, _teal, 'Communication Log',
          ...
          const SizedBox(height: 10),
          _buildCapabilityCard(context, isDark, Icons.do_not_disturb_on_rounded, _rose, 'Noise Filter & Mute Rules',
          ...
          const SizedBox(height: 10),
          _buildCapabilityCard(context, isDark, Icons.mark_chat_unread_rounded, _amber, 'Response Commitments',
  ...
  Widget _buildHeroHeader(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_sky.withOpacity(0.14), _indigo.withOpacity(0.08)],
      ...
      child: Column(children: [
        Row(children: [
          Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(
            color: _sky.withOpacity(0.15), borderRadius: BorderRadius.circular(14)),
            child: const Icon(Icons.forum_rounded, color: _sky, size: 28)),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('COMMUNICATION CENTRE', style: TextStyle(fontSize: 10, color: _sky, fontWeight: FontWeight.bold, letterSpacing: 2)),
            const SizedBox(height: 4),
            Text('Keep every exchange intentional — choose the right channel, set the right tone, and maintain a cadence that informs without overwhelming.',
  ...
  Widget _buildMiniStat(String value, String label, Color color) {
    return Expanded(child: Column(children: [
      ...
      Text(label, textAlign: TextAlign.center, style: TextStyle(fontSize: 8, color: color.withOpacity(0.75), letterSpacing: 0.3, height: 1.2)),
    ]));
  }
      ...
      children: channels.map((c) => Container(
        ...
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          Text(c.$2, textAlign: TextAlign.center,
              style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: onSurface, height: 1.2)),
          const SizedBox(height: 2),
          Text(c.$3, textAlign: TextAlign.center, style: TextStyle(fontSize: 8, color: c.$4, letterSpacing: 0.3)),
        ]),
      )).toList(),
    );
  }
      ...
      child: Column(children: metrics.map((m) => Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Text(m.$2, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: m.$4)),
          ]),
          const SizedBox(height: 4),
          Stack(children: [
            Container(height: 5, decoration: BoxDecoration(color: Colors.white.withOpacity(0.07), borderRadius: BorderRadius.circular(3))),
            FractionallySizedBox(widthFactor: m.$3, child: Container(height: 5, decoration: BoxDecoration(
              color: m.$4, borderRadius: BorderRadius.circular(3),
              boxShadow: [BoxShadow(color: m.$4.withOpacity(0.35), blurRadius: 4)],
            ))),
          ]),
        ]),
      )).toList()),
  ...
  Widget _buildCapabilityCard(BuildContext context, bool isDark, IconData icon, Color color, String title, String body) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
  ...
  Widget _buildCommunicationPrinciples(BuildContext context, bool isDark, Color onSurface) {
    final principles = [
      ('Right Channel', 'Match the message type to the channel — urgent matters belong in direct messages, not broadcast feeds.', Icons.swap_horiz_rounded, _sky),
      ('Right Time', 'Send messages when the recipient can act on them, not when it is convenient for you to write them.', Icons.schedule_rounded, _blue),
      ('Right Tone', 'Calibrate formality, directness, and warmth to the relationship and context, not personal habit.', Icons.record_voice_over_rounded, _indigo),
    ];
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_sky.withOpacity(0.08), _indigo.withOpacity(0.06)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(16),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const Text('3 PRINCIPLES OF EFFECTIVE COMMUNICATION', style: TextStyle(fontSize: 9, fontWeight: FontWeight.bold, letterSpacing: 1.5, color: _amber)),
        ]),
        const SizedBox(height: 12),
        ...principles.map((p) => Padding(
          padding: const EdgeInsets.only(bottom: 8),
          child: Row(children: [
            Icon(p.$4, color: p.$4, size: 18),
            const SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(p.$1, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: onSurface)),
              Text(p.$2, style: TextStyle(fontSize: 11, color: onSurface.withOpacity(0.65), height: 1.3)),
            ])),
          ]),
        )),
      ]),
