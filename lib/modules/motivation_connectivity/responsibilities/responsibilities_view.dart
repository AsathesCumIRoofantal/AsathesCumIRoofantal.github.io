...
class ResponsibilitiesView extends GetView<ResponsibilitiesController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('RESPONSIBILITIES', style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold, fontSize: 14)),
        centerTitle: true,
        actions: [Padding(
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildSectionLabel('DUTY DOMAINS', Icons.category_rounded, _sage, onSurface),
          const SizedBox(height: 12),
          _buildDomainRings(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('RESPONSIBILITY DETAILS', Icons.list_alt_rounded, _earth, onSurface),
          const SizedBox(height: 12),
          _buildRespCard(context, isDark, Icons.person_rounded, _sage, 'Self-Responsibility Inventory',
          ...
          _buildRespCard(context, isDark, Icons.family_restroom_rounded, _earth, 'Family Duty Map',
          ...
          const SizedBox(height: 10),
          _buildRespCard(context, isDark, Icons.work_rounded, _terracotta, 'Professional Obligations Tracker',
          ...
          const SizedBox(height: 10),
          _buildRespCard(context, isDark, Icons.groups_rounded, _moss, 'Community Contribution Planner',
          ...
          const SizedBox(height: 10),
          _buildRespCard(context, isDark, Icons.balance_rounded, _sky, 'Responsibility Conflict Resolver',
          ...
          const SizedBox(height: 10),
          _buildRespCard(context, isDark, Icons.share_rounded, _violet, 'Duty Delegation Guide',
          ...
          const SizedBox(height: 10),
          _buildRespCard(context, isDark, Icons.loop_rounded, _amber, 'Responsibility Review Cycle',
          ...
          const SizedBox(height: 24),
          _buildSectionLabel('RESPONSIBILITY LOAD', Icons.bar_chart_rounded, _amber, onSurface),
          const SizedBox(height: 12),
  ...
  Widget _buildHeroHeader(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: _sage.withOpacity(0.12), blurRadius: 16, offset: const Offset(0, 6))],
      ),
      child: Column(children: [
        Row(children: [
          Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(
            color: _sage.withOpacity(0.15), borderRadius: BorderRadius.circular(14)),
            child: const Icon(Icons.assignment_rounded, color: _sage, size: 28)),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('DUTY MAP', style: TextStyle(fontSize: 10, color: _sage, fontWeight: FontWeight.bold, letterSpacing: 2)),
            const SizedBox(height: 4),
            const Text('Map the full landscape of what you owe — to yourself, your family, your work, and your community — so nothing important falls through the cracks.',
  ...
  Widget _buildDomainRings(BuildContext context, bool isDark, Color onSurface) {
    final domains = [
      (Icons.person_rounded, 'SELF', 'Health · Growth · Finances · Wellbeing', _sage, '5 duties'),
      (Icons.family_restroom_rounded, 'FAMILY', 'Care · Presence · Support · Decisions', _earth, '4 duties'),
      (Icons.work_rounded, 'WORK', 'Deliverables · Relationships · Role Duties', _terracotta, '6 duties'),
      (Icons.groups_rounded, 'COMMUNITY', 'AIR · Local · Broader Contribution', _moss, '3 duties'),
    ];
    return Column(children: domains.map((d) => Container(
      margin: const EdgeInsets.only(bottom: 10),
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(
        color: d.$4.withOpacity(0.07), borderRadius: BorderRadius.circular(14),
        border: Border.all(color: d.$4.withOpacity(0.22)),
      ),
      child: Row(children: [
        Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(
          color: d.$4.withOpacity(0.15), borderRadius: BorderRadius.circular(12)),
          child: Icon(d.$1, color: d.$4, size: 22)),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(d.$2, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, letterSpacing: 1.5, color: d.$4)),
          const SizedBox(height: 2),
          Text(d.$3, style: TextStyle(fontSize: 10, color: onSurface.withOpacity(0.58), height: 1.3)),
        ])),
        Container(
          padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
          decoration: BoxDecoration(color: d.$4.withOpacity(0.12), borderRadius: BorderRadius.circular(8)),
          child: Text(d.$5, style: TextStyle(fontSize: 9, fontWeight: FontWeight.bold, color: d.$4)),
        ),
      ]),
    )).toList());
  }
  ...
  Widget _buildLoadMeter(BuildContext context, bool isDark, Color onSurface) {
    ...
    return Container(
      ...
      child: Column(children: [
        Text('Current duty load across all four domains — aim for balance, not perfection.',
            style: TextStyle(fontSize: 11, color: onSurface.withOpacity(0.60), height: 1.3)),
        const SizedBox(height: 12),
        ...domains.map((d) => Padding(
          padding: const EdgeInsets.only(bottom: 10),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text(d.$1, style: TextStyle(fontSize: 12, color: onSurface)),
              Text('${(d.$2 * 100).round()}%', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: d.$3)),
            ]),
            const SizedBox(height: 4),
            Stack(children: [
              Container(height: 6, decoration: BoxDecoration(
                color: Colors.white.withOpacity(0.07), borderRadius: BorderRadius.circular(3))),
              FractionallySizedBox(widthFactor: d.$2, child: Container(height: 6, decoration: BoxDecoration(
                color: d.$3, borderRadius: BorderRadius.circular(3),
                boxShadow: [BoxShadow(color: d.$3.withOpacity(0.3), blurRadius: 4)],
              ))),
            ]),
          ]),
        )),
  ...
  Widget _buildQuoteBanner(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_sage.withOpacity(0.10), _moss.withOpacity(0.07)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(16),
