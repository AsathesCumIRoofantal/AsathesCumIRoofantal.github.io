...
const List<_HopeBet> _sampleBets = [
  _HopeBet(
    ...
    description: 'Applied for the senior position at the organisation. Interview went well, waiting for the final decision from the panel.',
    category: 'Career',
    ...
    emotionalNote: 'Feeling cautiously optimistic — trying not to over-invest emotionally.',
    accent: Color(0xFF7C4DFF),
    icon: Icons.work_outline_rounded,
  ),
  _HopeBet(
    title: 'Funding Proposal Submitted',
    description: 'Submitted the grant proposal for the community project. The committee meets next month to review all applications.',
    category: 'Finance',
    status: _HopeStatus.waiting,
    confidencePercent: 55,
    followUpDate: 'Follow up by Jun 15',
    parallelPath: 'Exploring crowdfunding as a backup if this does not come through.',
    emotionalNote: 'Neutral — prepared for either outcome and have a plan B ready.',
    accent: Color(0xFF00BCD4),
    icon: Icons.account_balance_outlined,
  ),
  _HopeBet(
    title: 'Partnership Conversation',
    description: 'Had a promising initial meeting with a potential partner organisation. Waiting for their internal approval.',
    category: 'Collaboration',
    status: _HopeStatus.nervous,
    confidencePercent: 40,
    followUpDate: 'Follow up by May 28',
    parallelPath: 'Two other potential partners identified if this one does not proceed.',
    emotionalNote: 'A little anxious — this one matters a lot. Reminding myself I have options.',
    accent: Color(0xFFFF9800),
    icon: Icons.handshake_outlined,
  ),
  _HopeBet(
    title: 'Medical Test Results',
    description: 'Routine health check done. Waiting for the lab results to come back from the clinic.',
    category: 'Health',
...
const List<Map<String, dynamic>> _featureSections = [
  {
    ...
    'desc': 'Log every decision, proposal, or application you\'re waiting on with a clear status. Attach context notes so you remember exactly what\'s at stake when the answer arrives.',
    'points': [
  ...
  },
  {
    ...
    'desc': 'Set smart reminders to follow up if you haven\'t heard back by a chosen date. A well-timed nudge keeps momentum without seeming pushy.',
    'points': [
  ...
  },
  {
    ...
    'desc': 'Rate your confidence level for each pending outcome so you can plan contingencies. Revisiting your estimates over time sharpens your intuition.',
    'points': [
  ...
  },
  {
    ...
    'desc': 'While one door is closed, keep others open — document your backup plans and alternatives. Healthy waiting means pursuing parallel options.',
    'points': [
  ...
  },
  {
    ...
    'desc': 'When results arrive, record what happened and what you learned from the wait. A growing archive becomes a personal calibration tool.',
    'points': [
  ...
  },
  {
    ...
    'desc': 'Waiting can be draining — log how you\'re feeling about each pending item to stay self-aware. Spotting anxiety spikes early lets you redirect energy.',
    'points': [
  ...
  },
  {
    ...
    'color': Color(0xFF00BCD4),
    'desc': 'Invite teammates to add their own pending outcomes to a shared board for visibility. When the team knows what everyone is waiting on, support happens naturally.',
    'points': [
...
class FingersAreCrossedView extends GetView<FingersAreCrossedController> {
  ...
  Widget _buildSliverAppBar(BuildContext context, bool isDark) {
    return SliverAppBar(
      ...
      floating: false,
      pinned: true,
      backgroundColor: isDark ? const Color(0xFF0D0B1A) : const Color(0xFF1A237E),
      ...
      bottom: TabBar(
        ...
        tabs: const [
          Tab(icon: Icon(Icons.pending_actions_outlined, size: 18), text: 'My Bets'),
                  ...
                  builder: (_, __) => Transform.scale(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                            color: Colors.amber.withOpacity(0.35), width: 1.5),
                        boxShadow: [
                          BoxShadow(
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      // Mini stats row
                      Row(children: [
                        _MiniStat(label: 'Open', value: '3', color: Colors.amber),
                        const SizedBox(width: 16),
                        _MiniStat(label: 'Resolved', value: '1', color: Colors.greenAccent),
                        const SizedBox(width: 16),
                        _MiniStat(label: 'Avg Hope', value: '72%', color: Colors.cyanAccent),
                      ]),
                    ],
                  ),
                ),
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        // Bet cards
        ..._sampleBets.map((bet) => _BetCard(bet: bet, isDark: isDark, onSurface: onSurface)),
...
class _OverallHopeMeter extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final openBets = _sampleBets.where((b) => b.status != _HopeStatus.resolved && b.status != _HopeStatus.archived).toList();
    final avg = openBets.isEmpty ? 0 : openBets.map((b) => b.confidencePercent).reduce((a, b) => a + b) ~/ openBets.length;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(22),
        boxShadow: [
          BoxShadow(color: Colors.black.withOpacity(0.15), blurRadius: 14, offset: const Offset(0, 5)),
        ],
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          ClipRRect(
            ...
            child: LinearProgressIndicator(
              ...
              valueColor: AlwaysStoppedAnimation<Color>(
                avg >= 70 ? Colors.greenAccent : avg >= 40 ? Colors.amber : Colors.redAccent,
              ),
            ),
          ),
          const SizedBox(height: 10),
          Row(
            ...
            children: [
              Text('${openBets.length} open bets',
                  style: TextStyle(color: Colors.white.withOpacity(0.6), fontSize: 11)),
              Text(
                avg >= 70 ? '🌟 Looking bright!' : avg >= 40 ? '🤞 Hang in there' : '💪 Stay strong',
...
class _BetCard extends StatefulWidget {
  ...
  final bool isDark;
  final Color onSurface;
  const _BetCard({required this.bet, required this.isDark, required this.onSurface});
...
class _BetCardState extends State<_BetCard> {
  ...
  Color get _statusColor {
    switch (widget.bet.status) {
      case _HopeStatus.hopeful: return const Color(0xFF7C4DFF);
      case _HopeStatus.waiting: return const Color(0xFF00BCD4);
      case _HopeStatus.nervous: return const Color(0xFFFF9800);
      case _HopeStatus.resolved: return const Color(0xFF4CAF50);
      case _HopeStatus.archived: return Colors.grey;
    }
  }

  String get _statusLabel {
    switch (widget.bet.status) {
      case _HopeStatus.hopeful: return '🌟 Hopeful';
      case _HopeStatus.waiting: return '⏳ Waiting';
      case _HopeStatus.nervous: return '😬 Nervous';
      case _HopeStatus.resolved: return '✅ Resolved';
      case _HopeStatus.archived: return '📦 Archived';
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isDark ? const Color(0xFF1E193B).withOpacity(0.75) : Colors.white,
          borderRadius: BorderRadius.circular(20),
          border: Border.all(color: bet.accent.withOpacity(isDark ? 0.3 : 0.2)),
          boxShadow: isDark ? [] : [
        ...
        child: Column(
          ...
          children: [
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: bet.accent.withOpacity(0.13),
                          ...
                          children: [
                            ...
                            Row(children: [
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  color: bet.accent.withOpacity(0.1),
                                  borderRadius: BorderRadius.circular(6),
                                ),
                                child: Text(bet.category,
                                    style: TextStyle(fontSize: 9, color: bet.accent,
                      ...
                      Icon(_expanded ? Icons.keyboard_arrow_up_rounded : Icons.keyboard_arrow_down_rounded,
                          color: onSurface.withOpacity(0.35), size: 20),
                    ],
                  ),
                  const SizedBox(height: 12),
                  // Confidence bar
                  Row(children: [
                    Text('Hope: ${bet.confidencePercent}%',
                        style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: bet.accent)),
                    ...
                    Expanded(
                      child: ClipRRect(
                        ...
                        child: LinearProgressIndicator(
                          ...
                          minHeight: 7,
                          backgroundColor: bet.accent.withOpacity(0.12),
                          valueColor: AlwaysStoppedAnimation<Color>(bet.accent),
                        ),
                      ),
                    ),
                  ]),
                  ...
                  Text(bet.description,
                      style: TextStyle(fontSize: 12.5, color: onSurface.withOpacity(0.65), height: 1.4),
            ...
            // Expanded details
            if (_expanded) ...[
              Divider(height: 1, color: bet.accent.withOpacity(0.15)),
              Padding(
                padding: const EdgeInsets.all(16),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _DetailRow(icon: Icons.calendar_today_outlined, color: bet.accent,
                        label: 'Follow-Up', value: bet.followUpDate),
                    const SizedBox(height: 10),
                    _DetailRow(icon: Icons.alt_route_outlined, color: bet.accent,
                        label: 'Parallel Path', value: bet.parallelPath),
                    const SizedBox(height: 10),
                    _DetailRow(icon: Icons.self_improvement_rounded, color: bet.accent,
                        label: 'Emotional Note', value: bet.emotionalNote),
                    const SizedBox(height: 14),
                    Row(children: [
                      Expanded(
                        child: OutlinedButton.icon(
                          onPressed: () {
                            Clipboard.setData(ClipboardData(text: '${bet.title}: ${bet.description}'));
                          ...
                          icon: Icon(Icons.copy_outlined, size: 14, color: bet.accent),
                          label: Text('Copy', style: TextStyle(fontSize: 12, color: bet.accent)),
                          style: OutlinedButton.styleFrom(
                            side: BorderSide(color: bet.accent.withOpacity(0.4)),
        ...
        Column(
          ...
          children: [
            ...
            SizedBox(
              ...
              child: Text(value,
                  style: TextStyle(fontSize: 12.5, color: onSurface.withOpacity(0.75), height: 1.4)),
            ),
          ],
        ),
      ],
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: Colors.amber.withOpacity(0.25), style: BorderStyle.solid),
      ),
      child: Row(
        children: [
          Container(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text('Log something you\'re waiting on and stay grounded while the answer is in the air.',
                    style: TextStyle(fontSize: 11.5, color: onSurface.withOpacity(0.55), height: 1.4)),
              ],
            ),
          ),
    ...
    return ListView(
      ...
      children: [
        ...
        Container(
          ...
          child: Column(
            children: [
              const Icon(Icons.format_quote_rounded, color: Colors.amber, size: 40),
              const SizedBox(height: 16),
              Text(
                '"${quote.text}"',
              ...
              Row(
                ...
                children: [
                  // Dots
                  ...List.generate(_hopeQuotes.length, (i) => Container(
                    margin: const EdgeInsets.symmetric(horizontal: 3),
                    width: i == _currentQuote ? 18 : 6,
                    height: 6,
                    decoration: BoxDecoration(
                      color: i == _currentQuote ? Colors.amber : Colors.white.withOpacity(0.25),
              ...
              Row(
                ...
                children: [
                  OutlinedButton.icon(
                    onPressed: () => setState(() =>
                        _currentQuote = (_currentQuote - 1 + _hopeQuotes.length) % _hopeQuotes.length),
                    icon: const Icon(Icons.arrow_back_rounded, size: 14, color: Colors.white70),
                    label: const Text('Prev', style: TextStyle(color: Colors.white70, fontSize: 12)),
                    style: OutlinedButton.styleFrom(
                  ...
                  ElevatedButton.icon(
                    onPressed: () => setState(() =>
                        _currentQuote = (_currentQuote + 1) % _hopeQuotes.length),
                    icon: const Icon(Icons.arrow_forward_rounded, size: 14, color: Colors.black),
                    label: const Text('Next', style: TextStyle(color: Colors.black, fontSize: 12)),
                    style: ElevatedButton.styleFrom(
                  ...
                  OutlinedButton.icon(
                    onPressed: () {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: const Text('Quote copied!'),
                            duration: const Duration(seconds: 1),
                            behavior: SnackBarBehavior.floating,
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                      );
                    },
                    icon: const Icon(Icons.copy_outlined, size: 14, color: Colors.white70),
        ...
        Row(children: [
          Container(padding: const EdgeInsets.all(7),
            ...
            child: const Icon(Icons.format_list_bulleted_rounded, color: Colors.amber, size: 16)),
          const SizedBox(width: 10),
          Text('All Wisdom', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: onSurface)),
        ]),
        const SizedBox(height: 12),
        ..._hopeQuotes.asMap().entries.map((e) => GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: e.key == _currentQuote
                  ? Colors.amber.withOpacity(isDark ? 0.12 : 0.08)
                  : (isDark ? const Color(0xFF1E193B).withOpacity(0.5) : Colors.white),
            ...
            child: Row(
              ...
              children: [
                Icon(Icons.format_quote_rounded,
                    color: e.key == _currentQuote ? Colors.amber : onSurface.withOpacity(0.3),
                    size: 18),
                const SizedBox(width: 10),
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(e.value.text,
                          style: TextStyle(fontSize: 12.5, color: onSurface.withOpacity(0.8),
                              height: 1.45, fontStyle: FontStyle.italic),
                          maxLines: 3, overflow: TextOverflow.ellipsis),
                      const SizedBox(height: 4),
                      Text('— ${e.value.author}',
                          style: TextStyle(fontSize: 10, color: Colors.amber.withOpacity(0.8),
                              fontWeight: FontWeight.bold)),
                    ],
                  ),
                ),
...
class _GroundingCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.all(18),
      decoration: BoxDecoration(
        color: const Color(0xFF9C27B0).withOpacity(isDark ? 0.12 : 0.07),
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: const Color(0xFF9C27B0).withOpacity(0.25)),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(children: [
            const Icon(Icons.self_improvement_rounded, color: Color(0xFF9C27B0), size: 20),
            const SizedBox(width: 8),
            Text('5-4-3-2-1 Grounding Exercise',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: onSurface)),
          ]),
          const SizedBox(height: 6),
          Text('When the wait feels overwhelming, try this quick grounding technique to return to the present moment.',
              style: TextStyle(fontSize: 11.5, color: onSurface.withOpacity(0.6), height: 1.4)),
          const SizedBox(height: 14),
          ...steps.map((s) => Padding(
            ...
            child: Row(children: [
              ...
              Expanded(child: Text(s.$1,
                  style: TextStyle(fontSize: 12.5, color: onSurface.withOpacity(0.75)))),
            ]),
          )),
        ],
      ),
    ...
    return ListView(
      ...
      children: [
        ...
        Container(
          padding: const EdgeInsets.all(14),
          decoration: BoxDecoration(
            color: Colors.amber.withOpacity(isDark ? 0.1 : 0.07),
            ...
            children: [
              ...
              Expanded(
                child: Text(
                  ...
                  style: TextStyle(fontSize: 12.5, height: 1.5, color: onSurface.withOpacity(0.8)),
                ),
              ),
            ],
          ),
        ...
        ..._featureSections.map((s) => _FeatureSectionCard(
          section: s,
          isDark: isDark,
          onSurface: onSurface,
        )),
...
class _FeatureSectionCard extends StatelessWidget {
  ...
  final bool isDark;
  final Color onSurface;
  const _FeatureSectionCard({required this.section, required this.isDark, required this.onSurface});
    ...
    return Container(
      ...
      child: Theme(
        ...
        child: ExpansionTile(
          ...
          children: (section['points'] as List<String>).map((p) => Padding(
...
class _HealthyWaitingTips extends StatelessWidget {
  ...
  static const _tips = [
    ('Control what you can', 'Focus your energy on actions within your reach. The outcome is not yours to control — your preparation and response are.', Icons.tune_rounded, Color(0xFF7C4DFF)),
    ('Set a decision deadline', 'Decide in advance: "If I haven\'t heard by [date], I will move forward with Plan B." This prevents indefinite waiting.', Icons.event_outlined, Color(0xFF00BCD4)),
    ('Limit rumination windows', 'Allow yourself 10 minutes a day to think about the pending outcome — then redirect. Rumination beyond that rarely helps.', Icons.timer_outlined, Color(0xFFFF9800)),
    ('Celebrate the courage to try', 'You put yourself out there. That alone is worth acknowledging, regardless of the outcome.', Icons.celebration_outlined, Color(0xFF4CAF50)),
    ('Keep moving forward', 'The best antidote to anxious waiting is purposeful action in other areas of your life. Keep building.', Icons.directions_run_rounded, Color(0xFFE91E63)),
  ];
      ...
      children: [
        Row(children: [
          Container(
            ...
            child: const Icon(Icons.tips_and_updates_outlined, color: Colors.amber, size: 16),
          ),
          const SizedBox(width: 10),
          Text('Healthy Waiting Tips',
              style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: onSurface)),
        ]),
        const SizedBox(height: 12),
        ..._tips.map((t) => Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(14),
            border: Border.all(color: t.$4.withOpacity(0.2)),
          ),
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
