...
## 7. What's still a TODO after this pass

- Virtual background (blur/replace) — needs a segmentation model
  (e.g. `google_mlkit_selfie_segmentation`) + frame compositing; WebRTC
  itself doesn't provide this.
- Cloud recording, live captions (`recording_service.dart`,
  `stt_service.dart`) — still stubs, unrelated to the RTC swap.
- Chat/polls/Q&A currently ride on the separate mock `RtmService`; they
  could be moved onto the same data channel as remote-control (the
  `_handleAppData` hook in the controller is already set up to route more
  message types the same way `_name` is handled now).
