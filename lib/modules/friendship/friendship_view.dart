...
class FriendshipView extends GetView<FriendshipController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('FRIENDSHIP', style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold, fontSize: 14)),
        centerTitle: true,
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildSectionLabel('FRIEND ROSTER', Icons.contacts_outlined, _pink, onSurface),
          const SizedBox(height: 12),
          _buildFriendCircles(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('FRIENDSHIP TOOLKIT', Icons.build_rounded, _warm, onSurface),
          const SizedBox(height: 12),
          _buildFriendCard(context, isDark, Icons.phone_outlined, _peach, 'Reach-Out Log',
          ...
          _buildFriendCard(context, isDark, Icons.compare_arrows_outlined, _blue, 'Reciprocity Check',
          ...
          const SizedBox(height: 10),
          _buildFriendCard(context, isDark, Icons.record_voice_over_outlined, _rose, 'Depth Conversations',
          ...
          const SizedBox(height: 10),
          _buildFriendCard(context, isDark, Icons.explore_outlined, _teal, 'Shared Experiences',
          ...
          const SizedBox(height: 10),
          _buildFriendCard(context, isDark, Icons.handshake_outlined, _amber, 'Support Given & Received',
          ...
          const SizedBox(height: 10),
          _buildFriendCard(context, isDark, Icons.contacts_outlined, _pink, 'Friend Roster',
  ...
  Widget _buildHeroHeader(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [const Color(0xFF3D1200), const Color(0xFF1A0800)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(22),
        border: Border.all(color: _peach.withOpacity(0.35)),
        boxShadow: [BoxShadow(color: _pink.withOpacity(0.10), blurRadius: 16)],
      ),
      child: Column(children: [
        Row(children: [
          Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(
            color: _pink.withOpacity(0.15), borderRadius: BorderRadius.circular(14)),
            child: const Icon(Icons.group_outlined, color: _pink, size: 28)),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('RELATIONSHIP MAINTENANCE', style: TextStyle(fontSize: 9, color: _pink, fontWeight: FontWeight.bold, letterSpacing: 2)),
            const SizedBox(height: 4),
            const Text('Friendships decay without maintenance — not from malice, but from neglect. Track reach-outs, check reciprocity, and ensure your closest relationships get the attention they deserve.',
  ...
  Widget _buildFriendCircles(BuildContext context, bool isDark, Color onSurface) {
    final circles = [
      ('Close Friends', '5', 'Weekly contact', _rose),
      ('Good Friends', '9', 'Monthly contact', _pink),
      ('Casual Contacts', '28', 'Occasional', _peach),
    ];
    return Row(children: circles.map((c) => Expanded(child: Container(
      margin: const EdgeInsets.symmetric(horizontal: 4),
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(
        color: c.$4.withOpacity(0.08), borderRadius: BorderRadius.circular(14),
        border: Border.all(color: c.$4.withOpacity(0.22)),
      ),
      child: Column(children: [
        Text(c.$2, style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: c.$4)),
        const SizedBox(height: 4),
        Text(c.$1, textAlign: TextAlign.center, style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: onSurface, height: 1.2)),
        const SizedBox(height: 2),
        Text(c.$3, textAlign: TextAlign.center, style: TextStyle(fontSize: 9, color: c.$4.withOpacity(0.80), height: 1.2)),
      ]),
    ))).toList());
  }

  Widget _buildFriendCard(BuildContext context, bool isDark, IconData icon, Color color, String title, String body) {
    return Container(
  ...
  Widget _buildFriendQuote(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_pink.withOpacity(0.10), _peach.withOpacity(0.07)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: _pink.withOpacity(0.20)),
      ),
      child: Row(children: [
