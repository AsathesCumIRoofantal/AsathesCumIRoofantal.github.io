...
class ShooterController extends GetxController {
  ...
  void restart() { score.value = 0; lives.value = 3; isOver.value = false; }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Stack(children: [
        ...
        // HUD
        Positioned(top: 12, left: 16, child: Obx(() {
          final c = Get.find<ShooterController>();
          return Row(children: [
            Text('⭐ ${c.score.value}',
                style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
            const SizedBox(width: 16),
            ...List.generate(c.lives.value, (_) => const Text('❤️', style: TextStyle(fontSize: 14))),
          ]);
        })),
        Positioned(top: 12, right: 16, child: IconButton(
        ...
        Obx(() {
          ...
          return Container(
            ...
            child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              const SizedBox(height: 8),
              TextButton(onPressed: Get.back, child: const Text('Exit', style: TextStyle(color: Colors.white54))),
            ])),
          );
        }),
      ])),
...
class _ShooterCanvasState extends State<_ShooterCanvas>
  ...
  void _shoot() {
    _bullets.add(Rect.fromCenter(
        center: Offset(_px, _py - _pH / 2 - 4), width: _bW, height: _bH));
  }
  ...
  void _tick(Duration elapsed) {
    ...
    if (!_initialized) { _px = size.width / 2; _py = size.height - 80; _initialized = true;
      for (var i = 0; i < 60; i++) _stars.add(Offset(_rng.nextDouble() * size.width, _rng.nextDouble() * size.height)); }

    // Player movement
    if (_keys.contains(LogicalKeyboardKey.arrowLeft))  _vx = -300;
    else if (_keys.contains(LogicalKeyboardKey.arrowRight)) _vx = 300;
    else _vx *= 0.85;
    ...
    if (_spawnTimer >= 1.1) {
      ...
      _enemies.add(Rect.fromCenter(center: Offset(ex, -_eH), width: _eW, height: _eH));
    }
    ...
    for (final e in _enemies.toList()) {
      for (final b in _bullets.toList()) {
        if (e.overlaps(b)) {
          _enemies.remove(e); _bullets.remove(b);
          ctrl.addScore(10); break;
        }
      }
      if (e.bottom > size.height) { _enemies.remove(e); ctrl.loseLife(); }
    }
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onHorizontalDragUpdate: (d) => _px = (_px + d.delta.dx).clamp(0, context.size?.width ?? 400),
      onTapDown: (_) => _shoot(),
      child: CustomPaint(
        painter: _ShooterPainter(
          px: _px, py: _py, bullets: List.of(_bullets),
          enemies: List.of(_enemies), stars: _stars,
        ),
...
class _ShooterPainter extends CustomPainter {
  const _ShooterPainter({required this.px, required this.py,
    required this.bullets, required this.enemies, required this.stars});
  final double px, py;
  ...
  void paint(Canvas c, Size s) {
    ...
    // Engine glow
    c.drawCircle(Offset(px, py + 14), 6,
      Paint()..color = Colors.blueAccent.withOpacity(.7)
             ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 8));
    // Bullets
    final bulletP = Paint()..color = const Color(0xFFFCD34D)
      ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 3);
    for (final b in bullets) c.drawRRect(RRect.fromRectAndRadius(b, const Radius.circular(3)), bulletP);
    // Enemies
    final enemyP = Paint()..color = const Color(0xFFEF4444);
    for (final e in enemies) {
      c.drawRRect(RRect.fromRectAndRadius(e, const Radius.circular(8)), enemyP);
      c.drawCircle(e.center, 6,
        Paint()..color = Colors.orangeAccent.withOpacity(.8)
               ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 6));
    }
  }

  @override bool shouldRepaint(covariant _ShooterPainter old) => true;
}
