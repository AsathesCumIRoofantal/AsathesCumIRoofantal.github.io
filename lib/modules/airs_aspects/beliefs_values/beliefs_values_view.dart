...
class BeliefsValuesView extends GetView<BeliefsValuesController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('BELIEFS & VALUES', style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold, fontSize: 14)),
        centerTitle: true,
        actions: [Padding(
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildSectionLabel('CORE VALUES DECLARATION', Icons.star_rounded, _amber, onSurface),
          const SizedBox(height: 12),
          _buildCoreValuesGrid(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('BELIEF FRAMEWORK', Icons.account_tree_rounded, _violet, onSurface),
          const SizedBox(height: 12),
          _buildBeliefCard(context, isDark, Icons.star_rounded, _amber, 'Core Values Declaration',
          ...
          _buildBeliefCard(context, isDark, Icons.account_tree_rounded, _violet, 'Belief Mapping',
          ...
          const SizedBox(height: 10),
          _buildBeliefCard(context, isDark, Icons.fact_check_rounded, _teal, 'Values-in-Action Log',
          ...
          const SizedBox(height: 10),
          _buildBeliefCard(context, isDark, Icons.compass_calibration_rounded, _indigo, 'Alignment Check',
          ...
          const SizedBox(height: 10),
          _buildBeliefCard(context, isDark, Icons.shield_rounded, _rose, 'Ethical Boundaries',
          ...
          const SizedBox(height: 10),
          _buildBeliefCard(context, isDark, Icons.psychology_rounded, _purple, 'Values Coaching',
          ...
          const SizedBox(height: 24),
          _buildSectionLabel('VALUES COMPASS', Icons.explore_rounded, _gold, onSurface),
          const SizedBox(height: 12),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [const Color(0xFF3D2A00).withOpacity(isDark ? 1 : 0.6), const Color(0xFF1A0050).withOpacity(isDark ? 1 : 0.5)],
      ...
      child: Row(children: [
        Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(
          color: _amber.withOpacity(0.15), borderRadius: BorderRadius.circular(14)),
          child: const Icon(Icons.balance_rounded, color: _amber, size: 28)),
        const SizedBox(width: 14),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('YOUR PRINCIPLES COMPASS', style: TextStyle(fontSize: 10, color: _amber, fontWeight: FontWeight.bold, letterSpacing: 2)),
          const SizedBox(height: 4),
          Text('Articulate what you genuinely stand for so AIR can align its coaching, matching, and recommendations with your actual principles — not generic defaults.',
              style: TextStyle(fontSize: 12, height: 1.4, color: isDark ? Colors.white70 : Colors.white.withOpacity(0.9))),
        ])),
      ]),
    );
  }
  ...
  Widget _buildCoreValuesGrid(BuildContext context, bool isDark, Color onSurface) {
    final values = [
      ('Integrity', Icons.verified_rounded, _teal, 'Act honestly even when unseen'),
      ('Curiosity', Icons.explore_rounded, _indigo, 'Stay open to being wrong'),
      ('Service', Icons.volunteer_activism_rounded, _violet, 'Contribute before you extract'),
      ('Excellence', Icons.workspace_premium_rounded, _amber, 'Raise the floor, not just the ceiling'),
      ('Courage', Icons.shield_rounded, _rose, 'Act on conviction under pressure'),
      ('Growth', Icons.trending_up_rounded, _teal, 'Never finish becoming better'),
    ];
    return GridView.count(
      ...
      children: values.map((v) => Container(
        ...
        decoration: BoxDecoration(
          color: v.$3.withOpacity(0.07), borderRadius: BorderRadius.circular(14),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
          Row(children: [
            ...
            const SizedBox(width: 8),
            Expanded(child: Text(v.$1, style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: onSurface))),
          ]),
          const SizedBox(height: 4),
          Text(v.$4, style: TextStyle(fontSize: 10, color: onSurface.withOpacity(0.58), height: 1.3)),
        ]),
      )).toList(),
    );
  }

  Widget _buildBeliefCard(BuildContext context, bool isDark, IconData icon, Color color, String title, String body) {
    return Container(
  ...
  Widget _buildCompassCard(BuildContext context, bool isDark, Color onSurface) {
    final dimensions = [
      ('Towards People', 'How do you relate to and think about others?', Icons.people_rounded, _violet),
      ('Towards Systems', 'How do you approach rules, structures, and institutions?', Icons.account_balance_rounded, _indigo),
      ('Towards Progress', 'What is your belief about change and growth?', Icons.trending_up_rounded, _teal),
      ('Towards Self', 'What do you believe you owe yourself?', Icons.person_rounded, _amber),
    ];
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
      ...
      child: Column(children: dimensions.map((d) => Padding(
        padding: const EdgeInsets.only(bottom: 10),
        child: Row(children: [
          Icon(d.$4, color: d.$4, size: 20),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(d.$1, style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: onSurface)),
            Text(d.$2, style: TextStyle(fontSize: 11, color: onSurface.withOpacity(0.60), height: 1.3)),
          ])),
        ]),
      )).toList()),
    );
  ...
  Widget _buildInfoBanner(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_amber.withOpacity(0.10), _violet.withOpacity(0.08)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(16),
