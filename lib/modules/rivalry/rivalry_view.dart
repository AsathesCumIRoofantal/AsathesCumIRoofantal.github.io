...
class RivalryView extends GetView<RivalryController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('RIVALRY', style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold, fontSize: 14)),
        centerTitle: true,
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildSectionLabel('RIVALRY HEALTH CHECK', Icons.favorite_border, _orange, onSurface),
          const SizedBox(height: 12),
          _buildHealthCheck(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('RIVALRY TOOLKIT', Icons.build_rounded, _amber, onSurface),
          const SizedBox(height: 12),
          _buildCard(context, isDark, Icons.person_search_outlined, _fire, 'Identify Your Rivals',
          ...
          _buildCard(context, isDark, Icons.trending_up_outlined, _amber, 'What They Do Better',
          ...
          const SizedBox(height: 10),
          _buildCard(context, isDark, Icons.warning_amber_outlined, _fire, 'Rivalry vs. Resentment',
          ...
          const SizedBox(height: 10),
          _buildCard(context, isDark, Icons.school_outlined, _blue, 'Learning from Losses',
          ...
          const SizedBox(height: 10),
          _buildCard(context, isDark, Icons.handshake_outlined, _green, 'Mutual Respect',
          ...
          const SizedBox(height: 10),
          _buildCard(context, isDark, Icons.timer_off_outlined, _lime, 'Rivalry Expiry',
  ...
  Widget _buildHeroHeader(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [const Color(0xFF2A0800), const Color(0xFF150400)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(22),
        border: Border.all(color: _fire.withOpacity(0.38)),
        boxShadow: [BoxShadow(color: _orange.withOpacity(0.14), blurRadius: 18)],
      ),
      child: Column(children: [
        Row(children: [
          Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(
            color: _fire.withOpacity(0.15), borderRadius: BorderRadius.circular(14)),
            child: const Icon(Icons.sports_score_outlined, color: _orange, size: 28)),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('HEALTHY COMPETITION', style: TextStyle(fontSize: 10, color: _orange, fontWeight: FontWeight.bold, letterSpacing: 2)),
            const SizedBox(height: 4),
            const Text('Healthy rivalry sharpens you — it raises your standard, exposes your gaps, and pushes you past comfortable plateaus. Channel competitive tension into growth rather than letting it corrode relationships.',
  ...
  Widget _buildHealthCheck(BuildContext context, bool isDark, Color onSurface) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: _amber.withOpacity(0.14)),
      ),
      child: Column(children: checks.map((c) => Padding(
        padding: const EdgeInsets.only(bottom: 8),
        child: Row(children: [
          Icon(c.$2 ? Icons.check_circle_rounded : Icons.warning_rounded, color: c.$3, size: 16),
          const SizedBox(width: 10),
          Expanded(child: Text(c.$1, style: TextStyle(fontSize: 12, color: onSurface))),
        ]),
      )).toList()),
    );
  }
  ...
  Widget _buildRivalryQuote(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_fire.withOpacity(0.10), _amber.withOpacity(0.07)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: _fire.withOpacity(0.20)),
      ),
      child: Row(children: [
