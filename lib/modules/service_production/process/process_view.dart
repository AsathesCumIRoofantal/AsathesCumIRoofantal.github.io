...
class ProcessView extends GetView<ProcessController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('PROCESS', style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold, fontSize: 14)),
        centerTitle: true,
        actions: [Padding(
          padding: const EdgeInsets.only(right: 16),
          child: Icon(Icons.settings_suggest_rounded, color: _orange, size: 22),
        )],
      ),
      body: ListView(
        ...
        children: [
          ...
          _buildSectionLabel('PIPELINE STATUS', Icons.dashboard_rounded, _orange, onSurface),
          const SizedBox(height: 12),
          _buildPipelineKanban(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('SLA TRACKER', Icons.timer_rounded, _red, onSurface),
          const SizedBox(height: 12),
          _buildSLAPanel(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('PROCESS CAPABILITIES', Icons.tune_rounded, _amber, onSurface),
          const SizedBox(height: 12),
          _buildCapCard(context, isDark, Icons.dashboard_rounded, _orange, 'Active Job Dashboard',
          ...
          _buildCapCard(context, isDark, Icons.account_tree_rounded, _amber, 'Process Rules Engine',
          ...
          const SizedBox(height: 10),
          _buildCapCard(context, isDark, Icons.person_pin_rounded, _blue, 'Owner Assignment',
          ...
          const SizedBox(height: 10),
          _buildCapCard(context, isDark, Icons.tune_rounded, _green, 'Throughput Optimiser',
          ...
          const SizedBox(height: 10),
          _buildCapCard(context, isDark, Icons.escalator_warning_rounded, _red, 'Escalation Pathways',
          ...
          const SizedBox(height: 10),
          _buildCapCard(context, isDark, Icons.speed_rounded, _violet, 'Process Health Metrics',
  ...
  Widget _buildHeroHeader(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: _orange.withOpacity(0.12), blurRadius: 16, offset: const Offset(0, 6))],
      ),
      child: Column(children: [
        Row(children: [
          Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(
            color: _orange.withOpacity(0.15), borderRadius: BorderRadius.circular(14)),
            child: const Icon(Icons.settings_suggest_rounded, color: _orange, size: 28)),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('PROCESS CONTROL CENTRE', style: TextStyle(fontSize: 10, color: _orange, fontWeight: FontWeight.bold, letterSpacing: 2)),
            const SizedBox(height: 4),
            const Text('Govern the middle state of every AIR work item — defining rules, assigning owners, and enforcing SLAs while work is actively underway.',
  ...
  Widget _buildDarkStat(String v, String l, Color c) => Expanded(child: Column(children: [
    Text(v, style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: c)),
    Text(l, textAlign: TextAlign.center, style: TextStyle(fontSize: 8, color: c.withOpacity(0.75), height: 1.2, letterSpacing: 0.3)),
  ]));
  ...
  Widget _buildPipelineKanban(BuildContext context, bool isDark, Color onSurface) {
    final stages = [
      ('Input\nQueue', '12', _blue),
      ('In\nProcess', '47', _orange),
      ('Review', '8', _amber),
    ...
    return Row(children: stages.map((s) => Expanded(
      child: Container(
        margin: const EdgeInsets.symmetric(horizontal: 4),
        padding: const EdgeInsets.symmetric(vertical: 14, horizontal: 8),
        decoration: BoxDecoration(
          color: s.$3.withOpacity(0.08), borderRadius: BorderRadius.circular(14),
          border: Border.all(color: s.$3.withOpacity(0.22)),
        ),
        child: Column(children: [
          Text(s.$2, style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: s.$3)),
          const SizedBox(height: 4),
          Text(s.$1, textAlign: TextAlign.center,
              style: TextStyle(fontSize: 9, color: s.$3.withOpacity(0.85), height: 1.2)),
        ]),
      ),
    )).toList());
  }
  ...
  Widget _buildSLAPanel(BuildContext context, bool isDark, Color onSurface) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('SLA breach prediction fires 20% before each deadline — act before a miss is recorded.',
        ...
        ...slas.map((s) => Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Stack(children: [
              ...
              FractionallySizedBox(widthFactor: s.$3, child: Container(height: 5, decoration: BoxDecoration(
                color: s.$4, borderRadius: BorderRadius.circular(3)))),
            ]),
          ]),
        )),
      ]),
  ...
  Widget _buildTipBanner(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_orange.withOpacity(0.10), _amber.withOpacity(0.08)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(16),
