...
class TogetherUnisonView extends GetView<TogetherUnisonController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('TOGETHER IN UNISON', style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold, fontSize: 13)),
        centerTitle: true,
        actions: [Padding(
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildSectionLabel('UNISON HEALTH PULSE', Icons.favorite_rounded, _rose, onSurface),
          const SizedBox(height: 12),
          _buildPulsePanel(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('GROUP CAPABILITIES', Icons.settings_rounded, _pink, onSurface),
          const SizedBox(height: 12),
          _buildGroupCard(context, isDark, Icons.account_tree_rounded, _violet, 'Role Clarity Map',
          ...
          _buildGroupCard(context, isDark, Icons.calendar_month_rounded, _blue, 'Shared Rhythm Calendar',
          ...
          const SizedBox(height: 10),
          _buildGroupCard(context, isDark, Icons.handshake_rounded, _rose, 'Conflict Repair Protocol',
          ...
          const SizedBox(height: 10),
          _buildGroupCard(context, isDark, Icons.leaderboard_rounded, _teal, 'Contribution Visibility Board',
          ...
          const SizedBox(height: 10),
          _buildGroupCard(context, isDark, Icons.how_to_vote_rounded, _coral, 'Group Decision Framework',
          ...
          const SizedBox(height: 10),
          _buildGroupCard(context, isDark, Icons.favorite_rounded, _pink, 'Unison Health Pulse',
          ...
          const SizedBox(height: 24),
          _buildSectionLabel('WHAT UNISON IS NOT', Icons.info_outline_rounded, _amber, onSurface),
          const SizedBox(height: 12),
  ...
  Widget _buildHeroHeader(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: _rose.withOpacity(0.40)),
        boxShadow: [BoxShadow(color: _rose.withOpacity(0.15), blurRadius: 18, offset: const Offset(0, 6))],
      ),
      child: Column(children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('GROUP HARMONY MODULE', style: TextStyle(fontSize: 10, color: _rose, fontWeight: FontWeight.bold, letterSpacing: 2)),
            const SizedBox(height: 4),
            const Text('Create harmony in groups by clarifying roles, establishing shared rhythms, and building the repair skills that keep teams intact through inevitable friction.',
      ...
      child: Column(children: dimensions.map((d) => Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Stack(children: [
            ...
            FractionallySizedBox(widthFactor: d.$2, child: Container(height: 5, decoration: BoxDecoration(
              color: d.$3, borderRadius: BorderRadius.circular(3),
              boxShadow: [BoxShadow(color: d.$3.withOpacity(0.35), blurRadius: 4)],
            ))),
          ]),
        ]),
      )).toList()),
  ...
  Widget _buildGroupCard(BuildContext context, bool isDark, IconData icon, Color color, String title, String body) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
  ...
  Widget _buildUniformityCard(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: _amber.withOpacity(0.07), borderRadius: BorderRadius.circular(16),
        border: Border.all(color: _amber.withOpacity(0.20)),
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Unison is the disciplined coordination of different strengths toward a common beat — not the suppression of difference to create conformity.',
            style: TextStyle(fontSize: 13, fontStyle: FontStyle.italic, height: 1.5, color: onSurface)),
        const SizedBox(height: 10),
        Row(children: [
          const Icon(Icons.close_rounded, color: _rose, size: 14),
          const SizedBox(width: 6),
          Expanded(child: Text('Not uniformity — not everyone thinking and acting identically.', style: TextStyle(fontSize: 11, color: onSurface.withOpacity(0.70)))),
        ]),
        const SizedBox(height: 4),
        Row(children: [
          const Icon(Icons.check_rounded, color: _teal, size: 14),
          const SizedBox(width: 6),
          Expanded(child: Text('Coordinated diversity — different strengths, single direction.', style: TextStyle(fontSize: 11, color: onSurface.withOpacity(0.70)))),
        ]),
      ]),
    );
  }
