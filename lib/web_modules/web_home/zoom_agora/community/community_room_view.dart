...
class _CommunityRoomViewState extends State<CommunityRoomView> {
  ...
  void _send() {
    ...
    _input.clear();
    setState(() { _replyToId = null; _replyPreview = null; _editingMsgId = null; });
  ...
  Future<void> _pickAndSendFile() async {
    ...
    try {
      ...
      final bytes = f.bytes;
      if (bytes == null) {
        Get.snackbar('File error', 'Could not read file bytes.');
        return;
      }
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        Expanded(
          child: Obx(() {
            if (_c.isLoadingMsgs.value && _c.messages.isEmpty) {
              return const Center(child: CircularProgressIndicator(color: ZoomTheme.primary));
            }
        ...
        if (_replyPreview != null)
          _ReplyBanner(preview: _replyPreview!, onCancel: () =>
              setState(() { _replyToId = null; _replyPreview = null; })),
        if (_editingMsgId != null)
          _ReplyBanner(preview: 'Editing message…', isEdit: true,
              onCancel: () => setState(() { _editingMsgId = null; _input.clear(); })),
        // Composer
        _Composer(
          controller: _input,
          onSend: _send,
...
class _MessageBubble extends StatelessWidget {
  const _MessageBubble({
    ...
    required this.onReply, required this.onEdit,
    required this.onDelete, required this.onHistory,
  });
  ...
  Widget build(BuildContext context) {
    ...
    final time    = ms == 0 ? '' : _fmtTime(DateTime.fromMillisecondsSinceEpoch(ms));
              ...
              children: [
                if (!isMe)
                  Padding(padding: const EdgeInsets.only(bottom: 2),
                      child: Text(name, style: ZoomTheme.muted.copyWith(fontSize: 11))),
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: deleted
                        ? ZoomTheme.surface2
                        : isMe ? ZoomTheme.primary : ZoomTheme.surface2,
                  ...
                  child: deleted
                    ? Text('This message was deleted',
                        style: ZoomTheme.muted.copyWith(fontStyle: FontStyle.italic))
                ...
                Row(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  if (edited) ...[
                    ...
                    Text('edited', style: ZoomTheme.muted.copyWith(fontSize: 10, fontStyle: FontStyle.italic)),
                  ],
                ]),
              ],
            )),
  ...
  void _showMenu(BuildContext context) {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Wrap(children: [
        ListTile(leading: const Icon(Icons.reply, color: Colors.white),
          title: const Text('Reply', style: TextStyle(color: Colors.white)),
          onTap: () { Get.back(); onReply(id, text.length > 40 ? '${text.substring(0, 40)}…' : text); }),
        ListTile(leading: const Icon(Icons.copy, color: Colors.white),
          title: const Text('Copy', style: TextStyle(color: Colors.white)),
          onTap: () { Get.back(); Clipboard.setData(ClipboardData(text: text)); }),
        if (isMe) ...[
          ListTile(leading: const Icon(Icons.edit, color: Colors.white),
            title: const Text('Edit', style: TextStyle(color: Colors.white)),
            onTap: () { Get.back(); onEdit(id, text); }),
          ListTile(leading: const Icon(Icons.history, color: Colors.white),
            title: const Text('View history', style: TextStyle(color: Colors.white)),
            onTap: () { Get.back(); onHistory(id); }),
          ListTile(leading: const Icon(Icons.delete_outline, color: Color(0xFFFF5C7A)),
            title: const Text('Delete', style: TextStyle(color: Color(0xFFFF5C7A))),
            onTap: () { Get.back(); onDelete(id); }),
        ],
      ]),
    );
  }
...
class _ReplyBanner extends StatelessWidget {
  const _ReplyBanner({required this.preview, required this.onCancel, this.isEdit = false});
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(children: [
      Container(width: 3, height: 32, color: ZoomTheme.primary, margin: const EdgeInsets.only(right: 10)),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(isEdit ? 'Editing' : 'Replying to',
            style: const TextStyle(color: ZoomTheme.primary, fontSize: 11, fontWeight: FontWeight.w600)),
        Text(preview, style: ZoomTheme.muted.copyWith(fontSize: 12), overflow: TextOverflow.ellipsis),
      ])),
      IconButton(icon: const Icon(Icons.close, color: ZoomTheme.textMuted, size: 18), onPressed: onCancel),
    ]),
  );
}
      ...
      if (history.isEmpty)
        ...
        ...history.map((h) => Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(h['old_content'] as String? ?? '', style: const TextStyle(color: Colors.white)),
            const SizedBox(height: 2),
            Text(_fmtTs(h['edited_at'] as int? ?? 0), style: ZoomTheme.muted.copyWith(fontSize: 11)),
