...
class AppUtils {
  ...
  static String formatTime(int epochMs) => DateFormat(
    'hh:mm a',
  ).format(DateTime.fromMillisecondsSinceEpoch(epochMs));

  static String formatDateTime(int epochMs) => DateFormat(
    'MMM d, yyyy • hh:mm a',
  ).format(DateTime.fromMillisecondsSinceEpoch(epochMs));
  ...
  static bool isValidOtp(String v) => RegExp(r'^\d{6}$').hasMatch(v);
