...
class KindnessView extends GetView<KindnessController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('KINDNESS', style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold, fontSize: 14)),
        centerTitle: true,
        actions: [Padding(padding: const EdgeInsets.only(right: 16),
          child: Icon(Icons.volunteer_activism_outlined, color: _rose, size: 22))],
      ),
      body: ListView(
        ...
        children: [
          ...
          _buildSectionLabel('TODAY\'S KINDNESS LOG', Icons.wb_sunny_outlined, _coral, onSurface),
          const SizedBox(height: 12),
          _buildKindnessLog(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('KINDNESS PRACTICES', Icons.spa_rounded, _pink, onSurface),
          const SizedBox(height: 12),
          _buildKindCard(context, isDark, Icons.wb_sunny_outlined, _coral, 'Daily Kindness Habit',
          ...
          _buildKindCard(context, isDark, Icons.sentiment_satisfied_alt_outlined, _amber, 'Politeness Defaults',
          ...
          const SizedBox(height: 10),
          _buildKindCard(context, isDark, Icons.self_improvement_outlined, _violet, 'Kindness Under Pressure',
          ...
          const SizedBox(height: 10),
          _buildKindCard(context, isDark, Icons.visibility_outlined, _sky, 'Noticing Others',
          ...
          const SizedBox(height: 10),
          _buildKindCard(context, isDark, Icons.chat_outlined, _teal, 'Words That Land Well',
          ...
          const SizedBox(height: 10),
          _buildKindCard(context, isDark, Icons.spa_outlined, _rose, 'Kindness to Yourself',
  ...
  Widget _buildHeroHeader(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [const Color(0xFF3D000C), const Color(0xFF1A0006)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(22),
        border: Border.all(color: _rose.withOpacity(0.38)),
        boxShadow: [BoxShadow(color: _rose.withOpacity(0.12), blurRadius: 16)],
      ),
      child: Column(children: [
        Row(children: [
          Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(
            color: _rose.withOpacity(0.15), borderRadius: BorderRadius.circular(14)),
            child: const Icon(Icons.volunteer_activism_outlined, color: _rose, size: 28)),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('OPERATIONAL KINDNESS', style: TextStyle(fontSize: 9, color: _rose, fontWeight: FontWeight.bold, letterSpacing: 2)),
            const SizedBox(height: 4),
            const Text('Kindness in AIR is treated as an operational habit, not a personality trait. Build consistent, deliberate acts of consideration into daily interactions — not as performance, but as practice.',
  ...
  Widget _buildKindnessLog(BuildContext context, bool isDark, Color onSurface) {
    final acts = [
      ('Genuine compliment to a colleague', Icons.sentiment_very_satisfied_rounded, _pink),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: _rose.withOpacity(0.06), borderRadius: BorderRadius.circular(16),
        border: Border.all(color: _rose.withOpacity(0.18)),
      ),
      child: Column(children: acts.asMap().entries.map((e) {
        final a = e.value;
        return Padding(
          padding: const EdgeInsets.only(bottom: 8),
          child: Row(children: [
            Container(padding: const EdgeInsets.all(6), decoration: BoxDecoration(
              color: a.$3.withOpacity(0.12), borderRadius: BorderRadius.circular(8)),
              child: Icon(a.$2, color: a.$3, size: 14)),
            const SizedBox(width: 10),
            Expanded(child: Text(a.$1, style: TextStyle(fontSize: 12, color: onSurface))),
  ...
  Widget _buildKindCard(BuildContext context, bool isDark, IconData icon, Color color, String title, String body) {
    return Container(
  ...
  Widget _buildKindnessQuote(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_rose.withOpacity(0.10), _coral.withOpacity(0.07)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
