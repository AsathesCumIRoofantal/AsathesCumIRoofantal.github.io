...
class PracticeExpertiseView extends GetView<PracticeExpertiseController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('PRACTICE EXPERTISE', style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold, fontSize: 13)),
        centerTitle: true,
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildSectionLabel('SKILL DEPTH TRACKER', Icons.stacked_bar_chart_rounded, _indigo, onSurface),
          const SizedBox(height: 12),
          _buildDepthTracker(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('RECENT SESSIONS', Icons.edit_calendar_rounded, _violet, onSurface),
          const SizedBox(height: 12),
          _buildSessionLog(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('EXPERTISE TOOLS', Icons.build_rounded, _blue, onSurface),
          const SizedBox(height: 12),
          _buildExpertCard(context, isDark, Icons.edit_calendar_rounded, _indigo, 'Practice Session Log',
          ...
          _buildExpertCard(context, isDark, Icons.rate_review_rounded, _violet, 'Feedback Journal',
          ...
          const SizedBox(height: 10),
          _buildExpertCard(context, isDark, Icons.map_rounded, _blue, 'Specialisation Roadmap',
          ...
          const SizedBox(height: 10),
          _buildExpertCard(context, isDark, Icons.fitness_center_rounded, _sky, 'Rep Counter & Goals',
          ...
          const SizedBox(height: 10),
          _buildExpertCard(context, isDark, Icons.leaderboard_rounded, _cyan, 'Expert Benchmark Library',
          ...
          const SizedBox(height: 10),
          _buildExpertCard(context, isDark, Icons.workspace_premium_rounded, _amber, 'Mastery Certificates',
  ...
  Widget _buildHeroHeader(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [const Color(0xFF0E0840), const Color(0xFF060420)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(22),
        border: Border.all(color: _indigo.withOpacity(0.38)),
        boxShadow: [BoxShadow(color: _violet.withOpacity(0.12), blurRadius: 16)],
      ),
      child: Column(children: [
        Row(children: [
          Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(
            color: _indigo.withOpacity(0.15), borderRadius: BorderRadius.circular(14)),
            child: const Icon(Icons.model_training_rounded, color: _violet, size: 28)),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('DELIBERATE PRACTICE SYSTEM', style: TextStyle(fontSize: 9, color: _violet, fontWeight: FontWeight.bold, letterSpacing: 2)),
            const SizedBox(height: 4),
            const Text('Expertise is not accidental — it is the cumulative result of intentional reps recorded and reviewed over time. Log deliberate practice sessions and deepen specialisation through structured feedback loops.',
  ...
  Widget _buildDepthTracker(BuildContext context, bool isDark, Color onSurface) {
    final skills = [
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: _indigo.withOpacity(0.14)),
      ),
      child: Column(children: skills.map((s) => Padding(
        padding: const EdgeInsets.only(bottom: 10),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Text(s.$1, style: TextStyle(fontSize: 12, color: onSurface)),
            Text('${(s.$2 * 10).round()}/10', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: s.$3)),
          ]),
          const SizedBox(height: 4),
          Stack(children: [
            Container(height: 6, decoration: BoxDecoration(color: Colors.white.withOpacity(0.07), borderRadius: BorderRadius.circular(3))),
            FractionallySizedBox(widthFactor: s.$2, child: Container(height: 6, decoration: BoxDecoration(
  ...
  Widget _buildSessionLog(BuildContext context, bool isDark, Color onSurface) {
    final sessions = [
      ...
      ('Communication — Feedback Delivery', '45 min', '⭐⭐⭐⭐⭐', _sky, 'Yesterday'),
      ('Data Analysis — SQL Deep Dive', '60 min', '⭐⭐⭐', _cyan, '3 days ago'),
    ];
    return Column(children: sessions.map((s) => Container(
      margin: const EdgeInsets.only(bottom: 8),
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(
        color: s.$4.withOpacity(0.06), borderRadius: BorderRadius.circular(12),
        border: Border.all(color: s.$4.withOpacity(0.18)),
      ),
      child: Row(children: [
        Container(padding: const EdgeInsets.all(6), decoration: BoxDecoration(
          color: s.$4.withOpacity(0.12), borderRadius: BorderRadius.circular(8)),
          child: Icon(Icons.fitness_center_rounded, color: s.$4, size: 14)),
        const SizedBox(width: 10),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(s.$1, style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: onSurface)),
          Text('${s.$2} · ${s.$3}', style: TextStyle(fontSize: 10, color: onSurface.withOpacity(0.55))),
        ])),
        Text(s.$5, style: TextStyle(fontSize: 9, color: s.$4, fontWeight: FontWeight.w500)),
      ]),
    )).toList());
  }

  Widget _buildExpertCard(BuildContext context, bool isDark, IconData icon, Color color, String title, String body) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
  ...
  Widget _buildPracticeQuote(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_indigo.withOpacity(0.10), _violet.withOpacity(0.07)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: _indigo.withOpacity(0.20)),
      ),
      child: Row(children: [
