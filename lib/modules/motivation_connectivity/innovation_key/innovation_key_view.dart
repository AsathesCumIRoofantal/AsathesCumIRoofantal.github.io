...
class InnovationKeyView extends GetView<InnovationKeyController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('INNOVATION IS KEY', style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold, fontSize: 13)),
        centerTitle: true,
        actions: [Padding(
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildSectionLabel('INNOVATION PIPELINE', Icons.linear_scale_rounded, _amber, onSurface),
          const SizedBox(height: 12),
          _buildPipelineStages(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('IDEATION SPARKS', Icons.lightbulb_rounded, _yellow, onSurface),
          const SizedBox(height: 12),
          _buildIdeationGrid(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('CAPABILITIES', Icons.settings_rounded, _lime, onSurface),
          const SizedBox(height: 12),
          _buildCapCard(context, isDark, Icons.lightbulb_rounded, _yellow, 'Idea Generation Studio',
          ...
          _buildCapCard(context, isDark, Icons.verified_user_rounded, _orange, 'Integrity Stress Test',
          ...
          const SizedBox(height: 10),
          _buildCapCard(context, isDark, Icons.science_rounded, _cyan, 'Rapid Prototype Lab',
          ...
          const SizedBox(height: 10),
          _buildCapCard(context, isDark, Icons.analytics_rounded, _violet, 'Innovation Metrics',
          ...
          const SizedBox(height: 10),
          _buildCapCard(context, isDark, Icons.shuffle_rounded, _pink, 'Cross-Domain Pollination',
          ...
          const SizedBox(height: 10),
          _buildCapCard(context, isDark, Icons.emoji_events_rounded, _amber, 'Innovation Recognition',
  ...
  Widget _buildHeroHeader(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: _amber.withOpacity(0.40)),
        boxShadow: [BoxShadow(color: _amber.withOpacity(0.12), blurRadius: 16)],
      ),
      child: Column(children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('INNOVATION ENGINE', style: TextStyle(fontSize: 10, color: _yellow, fontWeight: FontWeight.bold, letterSpacing: 2)),
            const SizedBox(height: 4),
            const Text('Real innovation is disciplined creativity — it solves genuine problems in ways that hold up under scrutiny, not just under applause.',
  ...
  Widget _buildPipelineStages(BuildContext context, bool isDark, Color onSurface) {
    final stages = [
      ('Concept', '12', _yellow),
    ...
    return SingleChildScrollView(
      ...
      child: Row(children: stages.asMap().entries.map((e) {
        final s = e.value;
        return Row(children: [
          Container(
            width: 80,
            ...
            decoration: BoxDecoration(
              color: s.$3.withOpacity(0.10), borderRadius: BorderRadius.circular(14),
              border: Border.all(color: s.$3.withOpacity(0.25)),
            ),
            child: Column(children: [
              Text(s.$2, style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: s.$3)),
              const SizedBox(height: 4),
              Text(s.$1, textAlign: TextAlign.center, style: TextStyle(fontSize: 9, color: s.$3.withOpacity(0.85))),
            ]),
          ),
          if (e.key < stages.length - 1)
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 4),
              child: Icon(Icons.chevron_right_rounded, color: _amber.withOpacity(0.50), size: 20),
            ),
        ]);
      }).toList()),
    );
  ...
  Widget _buildIdeationGrid(BuildContext context, bool isDark, Color onSurface) {
    final sparks = [
    ...
    return GridView.count(
      ...
      children: sparks.map((s) => Container(
        padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
        decoration: BoxDecoration(
          color: s.$3.withOpacity(0.07), borderRadius: BorderRadius.circular(12),
          border: Border.all(color: s.$3.withOpacity(0.20)),
        ),
        child: Row(children: [
          Icon(s.$2, color: s.$3, size: 18),
          const SizedBox(width: 8),
          Expanded(child: Text(s.$1, style: TextStyle(fontSize: 10, color: onSurface, height: 1.2, fontWeight: FontWeight.w500))),
        ]),
      )).toList(),
    );
  }
  ...
  Widget _buildInnovationQuote(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_amber.withOpacity(0.10), _violet.withOpacity(0.07)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
