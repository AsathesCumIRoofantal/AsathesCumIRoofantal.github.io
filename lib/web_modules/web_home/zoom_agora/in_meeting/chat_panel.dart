  ...
  Widget build(BuildContext c) => Container(
    ...
    child: Row(children: [
      ...
      Container(
        ...
        child: const Text('Everyone', style: TextStyle(color: ZoomTheme.textMuted, fontSize: 11)),
      ),
      const Spacer(),
      IconButton(onPressed: () {}, icon: const Icon(Icons.search, color: ZoomTheme.textMuted, size: 18)),
      IconButton(onPressed: () {}, icon: const Icon(Icons.more_vert, color: ZoomTheme.textMuted, size: 18)),
    ]),
  );
}
  ...
  Widget build(BuildContext c) {
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          if (!isMe) InitialsAvatar(name: m.fromName, colorHex: color, size: 32),
          if (!isMe) const SizedBox(width: 10),
          Flexible(child: Column(
            crossAxisAlignment: isMe ? CrossAxisAlignment.end : CrossAxisAlignment.start,
            children: [
              Row(mainAxisSize: MainAxisSize.min, children: [
                Text(isMe ? 'You' : m.fromName,
                  style: ZoomTheme.body.copyWith(fontWeight: FontWeight.w600, fontSize: 13)),
                const SizedBox(width: 8),
                Text(_time(m.sentAt), style: ZoomTheme.muted.copyWith(fontSize: 11)),
                if (m.scope == ChatScope.direct) ...[
                  const SizedBox(width: 6),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 1),
                    decoration: BoxDecoration(color: ZoomTheme.warn.withOpacity(.18),
                      borderRadius: BorderRadius.circular(6)),
                    child: const Text('DM', style: TextStyle(color: ZoomTheme.warn, fontSize: 10, fontWeight: FontWeight.w700)),
                  ),
                ],
              ]),
    ...
    child: Column(children: [
      Obx(() => Row(children: [
        const Padding(padding: EdgeInsets.symmetric(horizontal: 8),
          child: Text('To:', style: TextStyle(color: ZoomTheme.textMuted, fontSize: 12))),
        for (final s in ChatScope.values)
          Padding(
            ...
            child: ChoiceChip(
              ...
              labelStyle: TextStyle(color: scope.value == s ? Colors.white : ZoomTheme.textMuted),
            ),
          ),
      ])),
