...
class LiveFullestView extends GetView<LiveFullestController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('LIVE TO THE FULLEST', style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold, fontSize: 13)),
        centerTitle: true,
        actions: [Padding(
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildSectionLabel('8 LIFE DOMAINS', Icons.radar_rounded, _sky, onSurface),
          const SizedBox(height: 12),
          _buildDomainGrid(context, isDark, onSurface),
          const SizedBox(height: 24),
          _buildSectionLabel('FULLEST LIVING TOOLS', Icons.bolt_rounded, _purple, onSurface),
          const SizedBox(height: 12),
          _buildToolCard(context, isDark, Icons.radar_rounded, _sky, 'Life Domain Audit',
          ...
          _buildToolCard(context, isDark, Icons.bolt_rounded, _amber, 'Energy Architecture',
          ...
          const SizedBox(height: 10),
          _buildToolCard(context, isDark, Icons.explore_rounded, _teal, 'Ethical Compass Check',
          ...
          const SizedBox(height: 10),
          _buildToolCard(context, isDark, Icons.sentiment_very_satisfied_rounded, _pink, 'Joy Inventory',
          ...
          const SizedBox(height: 10),
          _buildToolCard(context, isDark, Icons.landscape_rounded, _purple, 'Whole-Life Vision Board',
          ...
          const SizedBox(height: 10),
          _buildToolCard(context, isDark, Icons.hourglass_top_rounded, _rose, 'Regret Minimisation Frame',
          ...
          const SizedBox(height: 10),
          _buildToolCard(context, isDark, Icons.wb_sunny_rounded, _orange, 'Daily Aliveness Ritual',
  ...
  Widget _buildHeroHeader(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: _sky.withOpacity(0.12), blurRadius: 18)],
      ),
      child: Column(children: [
        Row(children: [
          Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(
            color: _sky.withOpacity(0.15), borderRadius: BorderRadius.circular(14)),
            child: const Icon(Icons.self_improvement_rounded, color: _sky, size: 28)),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('WHOLE LIFE DESIGN', style: TextStyle(fontSize: 10, color: _sky, fontWeight: FontWeight.bold, letterSpacing: 2)),
            const SizedBox(height: 4),
            const Text('Design a whole life where energy, ethics, and joy occupy the same frame — not competing priorities but a single integrated vision.',
  ...
  Widget _buildDomainGrid(BuildContext context, bool isDark, Color onSurface) {
    ...
    return GridView.count(
      ...
      children: domains.map((d) => Container(
        ...
        decoration: BoxDecoration(
          color: d.$4.withOpacity(0.08), borderRadius: BorderRadius.circular(12),
          border: Border.all(color: d.$4.withOpacity(0.22)),
        ),
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(d.$1, color: d.$4, size: 18),
          const SizedBox(height: 4),
          Text(d.$2, textAlign: TextAlign.center, style: TextStyle(fontSize: 9, fontWeight: FontWeight.bold, color: onSurface, height: 1.1)),
          const SizedBox(height: 3),
          Text(d.$3, style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: d.$4)),
        ]),
      )).toList(),
    );
  }
  ...
  Widget _buildLifeQuote(BuildContext context, bool isDark, Color onSurface) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_sky.withOpacity(0.10), _purple.withOpacity(0.07)],
            begin: Alignment.topLeft, end: Alignment.bottomRight),
